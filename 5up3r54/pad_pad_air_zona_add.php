...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_zona_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_air_zona_inline_add.htm";
else
	$templatefile = "pad_pad_air_zona_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_air_zona_add.php');
...
//	processing nama - end
//	processing indeks - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_indeks = $pageObject->getControl("indeks", $id);
		$control_indeks->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_air_zona_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_air_zona_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_air_zona_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["indeks"]=@$avalues["indeks"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("indeks_label","<label for=\"".GetInputElementId("indeks", $id, PAGE_ADD)."\">","</label>");
