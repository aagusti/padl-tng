...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_stpd_export"] = $layout;
...
$xt->display("pad_pad_stpd_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_stpd.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_stpd.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_stpd.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["stpdno"] = $pageObject->showDBValue("stpdno", $row);
			$values["stpdtgl"] = $pageObject->showDBValue("stpdtgl", $row);
			$values["spt_id"] = $pageObject->showDBValue("spt_id", $row);
			$values["jatuhtempotgl"] = $pageObject->showDBValue("jatuhtempotgl", $row);
			$values["printed"] = $pageObject->showDBValue("printed", $row);
			$values["stpdcount"] = $pageObject->showDBValue("stpdcount", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_stpd.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["stpdno"] = $pageObject->getViewControl("stpdno")->showDBValue($row, "");
			$values["stpdtgl"] = $pageObject->getViewControl("stpdtgl")->showDBValue($row, "");
			$values["spt_id"] = $pageObject->getViewControl("spt_id")->showDBValue($row, "");
			$values["jatuhtempotgl"] = $pageObject->getViewControl("jatuhtempotgl")->showDBValue($row, "");
			$values["printed"] = $pageObject->getViewControl("printed")->showDBValue($row, "");
			$values["stpdcount"] = $pageObject->getViewControl("stpdcount")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["stpdno"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["stpdtgl"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["spt_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["jatuhtempotgl"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["printed"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["stpdcount"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["petugas_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["pejabat_id"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Stpdno").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Stpdtgl").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Spt Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Jatuhtempotgl").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Printed").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Stpdcount").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Stpdtgl"."</td>";
		echo "<td>"."Spt Id"."</td>";
		echo "<td>"."Jatuhtempotgl"."</td>";
		echo "<td>"."Printed"."</td>";
		echo "<td>"."Stpdcount"."</td>";
		echo "<td>"."Bunga"."</td>";
		echo "<td>"."Enabled"."</td>";
		echo "<td>"."Created"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["stpdtgl"]);
					else
						echo $values["stpdtgl"];
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["spt_id"]);
				else
					echo $values["spt_id"];//echo htmlspecialchars($values["spt_id"]); commented for bug #6823
					
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["jatuhtempotgl"]);
					else
						echo $values["jatuhtempotgl"];
			echo '</td>';
							echo '<td>';
