...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_daftar_hist_export"] = $layout;
...
$xt->display("pad_pad_daftar_hist_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_daftar_hist.xls");
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_daftar_hist.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["status_id"]).'"';
			if($outstr!="")
				$outstr.=",";
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["status_id"];//echo htmlspecialchars($values["status_id"]); commented for bug #6823
