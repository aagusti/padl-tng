...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_customer_detail_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_customer_detail_inline_edit.htm" : "pad_pad_customer_detail_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_customer_detail_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_customer_detail_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_customer_detail";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_customer_detail_view.php?".$viewlink."'\"");
