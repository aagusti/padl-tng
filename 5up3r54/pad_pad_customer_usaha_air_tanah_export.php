...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_customer_usaha_air_tanah_export"] = $layout;
...
$xt->display("pad_pad_customer_usaha_air_tanah_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_customer_usaha_air_tanah.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_customer_usaha_air_tanah.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_customer_usaha_air_tanah.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["customer_usaha_id"] = $pageObject->showDBValue("customer_usaha_id", $row);
			$values["sumur_ke"] = $pageObject->showDBValue("sumur_ke", $row);
			$values["sipa_no"] = $pageObject->showDBValue("sipa_no", $row);
			$values["max_volume"] = $pageObject->showDBValue("max_volume", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_customer_usaha_air_tanah.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["customer_usaha_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["sumur_ke"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["sipa_no"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["max_volume"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["disabled"]).'"';
			if($outstr!="")
				$outstr.=",";
