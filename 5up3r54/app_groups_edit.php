...
include("include/app_groups_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["app_groups_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "app_groups_inline_edit.htm" : "app_groups_edit.htm";
...
//	processing nama - end
//	processing locked - begin
	$condition = 1;

	if($condition)
	{
		$control_locked = $pageObject->getControl("locked", $id);
		$control_locked->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing locked - end
...
//	processing kode - end
	...
	//	processing nama - end
			//	processing locked - begin
				...
				if($condition)
				{
					$control_locked->afterSuccessfulSave();
				}
	//	processing locked - end
			//	processing kode - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: app_groups_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: app_groups_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("locked"))
		$xt->assign("locked_fieldblock",true);
	else
		$xt->assign("locked_tabfieldblock",true);
	$xt->assign("locked_label",true);
	if(isEnableSection508())
		$xt->assign_section("locked_label","<label for=\"".GetInputElementId("locked", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	$showDetailKeys["app.group_modules"]["masterkey1"] = $data["id"];		
...
//	locked - 
	$value = $pageObject->showDBValue("locked", $data, $keylink);
	$showValues["locked"] = $value;
	$showFields[] = "locked";
		$showRawValues["locked"] = substr($data["locked"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "app.groups";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "app.groups";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='app_groups_view.php?".$viewlink."'\"");
