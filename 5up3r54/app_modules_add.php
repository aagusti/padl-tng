...
$layout->blocks["top"][] = "details";$page_layouts["app_modules_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "app_modules_inline_add.htm";
else
	$templatefile = "app_modules_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: app_modules_add.php');
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="app.apps")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["app_id"]==""){
			$avalues["app_id"] = prepare_for_db("app_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'app_modules_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'app_modules_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: app_modules_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="app.apps")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["app_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "app.modules";
		$options['firstTime'] = 1;
