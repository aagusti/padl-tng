...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_report_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_report_inline_edit.htm" : "pad_pad_report_edit.htm";
...
//	processing spt_type_id - begin
	$condition = 1;

	if($condition)
	{
		$control_spt_type_id = $pageObject->getControl("spt_type_id", $id);
		$control_spt_type_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing spt_type_id - end
...
//	processing sptnm - begin
	$condition = 1;

	if($condition)
	{
		$control_sptnm = $pageObject->getControl("sptnm", $id);
		$control_sptnm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sptnm - end
//	processing sknm - begin
	$condition = 1;

	if($condition)
	{
		$control_sknm = $pageObject->getControl("sknm", $id);
		$control_sknm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sknm - end
//	processing nhnm - begin
	$condition = 1;

	if($condition)
	{
		$control_nhnm = $pageObject->getControl("nhnm", $id);
		$control_nhnm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nhnm - end
//	processing sspdnm - begin
	$condition = 1;

	if($condition)
	{
		$control_sspdnm = $pageObject->getControl("sspdnm", $id);
		$control_sspdnm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sspdnm - end
//	processing stpnm - begin
	$condition = 1;

	if($condition)
	{
		$control_stpnm = $pageObject->getControl("stpnm", $id);
		$control_stpnm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing stpnm - end
//	processing kartudtnm - begin
	$condition = 1;

	if($condition)
	{
		$control_kartudtnm = $pageObject->getControl("kartudtnm", $id);
		$control_kartudtnm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kartudtnm - end
	...
	//	do event
		...
		{		
			...
			//	processing spt_type_id - begin
				...
				if($condition)
				{
					$control_spt_type_id->afterSuccessfulSave();
				}
	//	processing spt_type_id - end
			//	processing usaha_id - begin
							$condition = 1;
	...
	//	processing usaha_id - end
			//	processing sptnm - begin
				...
				if($condition)
				{
					$control_sptnm->afterSuccessfulSave();
				}
	//	processing sptnm - end
			//	processing sknm - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_sknm->afterSuccessfulSave();
				}
	//	processing sknm - end
			//	processing nhnm - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_nhnm->afterSuccessfulSave();
				}
	//	processing nhnm - end
			//	processing sspdnm - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_sspdnm->afterSuccessfulSave();
				}
	//	processing sspdnm - end
			//	processing stpnm - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_stpnm->afterSuccessfulSave();
				}
	//	processing stpnm - end
			//	processing kartudtnm - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_kartudtnm->afterSuccessfulSave();
				}
	//	processing kartudtnm - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_report_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	$data["spt_type_id"] = $evalues["spt_type_id"];
	$data["usaha_id"] = $evalues["usaha_id"];
	$data["sptnm"] = $evalues["sptnm"];
	$data["sknm"] = $evalues["sknm"];
	$data["nhnm"] = $evalues["nhnm"];
	$data["sspdnm"] = $evalues["sspdnm"];
	$data["stpnm"] = $evalues["stpnm"];
	$data["kartudtnm"] = $evalues["kartudtnm"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("spt_type_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("spt_type_id_label","<label for=\"".GetInputElementId("spt_type_id", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("sptnm_label",true);
	if(isEnableSection508())
		$xt->assign_section("sptnm_label","<label for=\"".GetInputElementId("sptnm", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sknm"))
		$xt->assign("sknm_fieldblock",true);
	else
		$xt->assign("sknm_tabfieldblock",true);
	$xt->assign("sknm_label",true);
	if(isEnableSection508())
		$xt->assign_section("sknm_label","<label for=\"".GetInputElementId("sknm", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("nhnm"))
		$xt->assign("nhnm_fieldblock",true);
	else
		$xt->assign("nhnm_tabfieldblock",true);
	$xt->assign("nhnm_label",true);
	if(isEnableSection508())
		$xt->assign_section("nhnm_label","<label for=\"".GetInputElementId("nhnm", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("sspdnm"))
		$xt->assign("sspdnm_fieldblock",true);
	else
		$xt->assign("sspdnm_tabfieldblock",true);
	$xt->assign("sspdnm_label",true);
	if(isEnableSection508())
		$xt->assign_section("sspdnm_label","<label for=\"".GetInputElementId("sspdnm", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("stpnm"))
		$xt->assign("stpnm_fieldblock",true);
	else
		$xt->assign("stpnm_tabfieldblock",true);
	$xt->assign("stpnm_label",true);
	if(isEnableSection508())
		$xt->assign_section("stpnm_label","<label for=\"".GetInputElementId("stpnm", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kartudtnm_label","<label for=\"".GetInputElementId("kartudtnm", $id, PAGE_EDIT)."\">","</label>");
...
//	spt_type_id - 
	...
	$showValues["spt_type_id"] = $value;
	$showFields[] = "spt_type_id";
		$showRawValues["spt_type_id"] = substr($data["spt_type_id"],0,100);
...
//	sptnm - 
	...
	$showValues["sptnm"] = $value;
	$showFields[] = "sptnm";
		$showRawValues["sptnm"] = substr($data["sptnm"],0,100);

//	sknm - 
	$value = $pageObject->showDBValue("sknm", $data, $keylink);
	$showValues["sknm"] = $value;
	$showFields[] = "sknm";
		$showRawValues["sknm"] = substr($data["sknm"],0,100);

//	nhnm - 
	$value = $pageObject->showDBValue("nhnm", $data, $keylink);
	$showValues["nhnm"] = $value;
	$showFields[] = "nhnm";
		$showRawValues["nhnm"] = substr($data["nhnm"],0,100);

//	sspdnm - 
	$value = $pageObject->showDBValue("sspdnm", $data, $keylink);
	$showValues["sspdnm"] = $value;
	$showFields[] = "sspdnm";
		$showRawValues["sspdnm"] = substr($data["sspdnm"],0,100);

//	stpnm - 
	$value = $pageObject->showDBValue("stpnm", $data, $keylink);
	$showValues["stpnm"] = $value;
	$showFields[] = "stpnm";
		$showRawValues["stpnm"] = substr($data["stpnm"],0,100);

//	kartudtnm - 
	$value = $pageObject->showDBValue("kartudtnm", $data, $keylink);
	$showValues["kartudtnm"] = $value;
	$showFields[] = "kartudtnm";
		$showRawValues["kartudtnm"] = substr($data["kartudtnm"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_report_view.php?".$viewlink."'\"");
