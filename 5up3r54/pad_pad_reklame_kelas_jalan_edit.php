...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_kelas_jalan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_reklame_kelas_jalan_inline_edit.htm" : "pad_pad_reklame_kelas_jalan_edit.htm";
...
//	processing nama - end
//	processing nilai - begin
	$condition = 1;

	if($condition)
	{
		$control_nilai = $pageObject->getControl("nilai", $id);
		$control_nilai->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nilai - end
...
//	processing kriteria - begin
	$condition = 1;

	if($condition)
	{
		$control_kriteria = $pageObject->getControl("kriteria", $id);
		$control_kriteria->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kriteria - end
...
//	processing enabled - end
	...
	//	processing nama - end
			//	processing nilai - begin
				...
				if($condition)
				{
					$control_nilai->afterSuccessfulSave();
				}
	//	processing nilai - end
			//	processing update_uid - begin
							$condition = 1;
	...
	//	processing singkatan - end
			//	processing kriteria - begin
				...
				if($condition)
				{
					$control_kriteria->afterSuccessfulSave();
				}
	//	processing kriteria - end
			//	processing enabled - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_reklame_kelas_jalan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_reklame_kelas_jalan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["kriteria"] = $evalues["kriteria"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("nilai"))
		$xt->assign("nilai_fieldblock",true);
	else
		$xt->assign("nilai_tabfieldblock",true);
	$xt->assign("nilai_label",true);
	if(isEnableSection508())
		$xt->assign_section("nilai_label","<label for=\"".GetInputElementId("nilai", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("kriteria"))
		$xt->assign("kriteria_fieldblock",true);
	else
		$xt->assign("kriteria_tabfieldblock",true);
	$xt->assign("kriteria_label",true);
	if(isEnableSection508())
		$xt->assign_section("kriteria_label","<label for=\"".GetInputElementId("kriteria", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	$showDetailKeys["pad.pad_reklame_jalan"]["masterkey1"] = $data["id"];		
...
//	nilai - Number
	$value = $pageObject->showDBValue("nilai", $data, $keylink);
	$showValues["nilai"] = $value;
	$showFields[] = "nilai";
		$showRawValues["nilai"] = substr($data["nilai"],0,100);
...
//	kriteria - 
	$value = $pageObject->showDBValue("kriteria", $data, $keylink);
	$showValues["kriteria"] = $value;
	$showFields[] = "kriteria";
		$showRawValues["kriteria"] = substr($data["kriteria"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_reklame_kelas_jalan";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_reklame_kelas_jalan_view.php?".$viewlink."'\"");
