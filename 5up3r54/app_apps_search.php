...
include("include/app_apps_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["app_apps_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "app_apps_search.htm";
...
$params['shortTableName'] = 'app_apps';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("app_path"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("app_path")] = GetTableURL($pageObject->pSet->getLookupTable("app_path"));
	
	$pageObject->fillFieldToolTips("app_path");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("app_path");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "app_path";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("app_path_label","<label for=\"".GetInputElementId("app_path", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("app_path_label", true);
	
	$xt->assign("app_path_fieldblock", true);
	$xt->assignbyref("app_path_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("app_path_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("app_path_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_app_path", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("app_path");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"app_path", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"app_path", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
