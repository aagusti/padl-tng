...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_teguran_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_teguran_inline_edit.htm" : "pad_pad_teguran_edit.htm";
...
//	processing teguran_no - begin
	$condition = 1;

	if($condition)
	{
		$control_teguran_no = $pageObject->getControl("teguran_no", $id);
		$control_teguran_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing teguran_no - end
...
//	processing cetak_ke - begin
	$condition = 1;

	if($condition)
	{
		$control_cetak_ke = $pageObject->getControl("cetak_ke", $id);
		$control_cetak_ke->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing cetak_ke - end
...
//	processing update_uid - end
	...
	//	processing tanggal - end
			//	processing teguran_no - begin
				...
				if($condition)
				{
					$control_teguran_no->afterSuccessfulSave();
				}
	//	processing teguran_no - end
			//	processing invoice_id - begin
							$condition = 1;
	...
	//	processing keterangan - end
			//	processing cetak_ke - begin
				...
				if($condition)
				{
					$control_cetak_ke->afterSuccessfulSave();
				}
	//	processing cetak_ke - end
			//	processing created - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_teguran_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_teguran_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("teguran_no"))
		$xt->assign("teguran_no_fieldblock",true);
	else
		$xt->assign("teguran_no_tabfieldblock",true);
	$xt->assign("teguran_no_label",true);
	if(isEnableSection508())
		$xt->assign_section("teguran_no_label","<label for=\"".GetInputElementId("teguran_no", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("cetak_ke"))
		$xt->assign("cetak_ke_fieldblock",true);
	else
		$xt->assign("cetak_ke_tabfieldblock",true);
	$xt->assign("cetak_ke_label",true);
	if(isEnableSection508())
		$xt->assign_section("cetak_ke_label","<label for=\"".GetInputElementId("cetak_ke", $id, PAGE_EDIT)."\">","</label>");
...
//	teguran_no - 
	$value = $pageObject->showDBValue("teguran_no", $data, $keylink);
	$showValues["teguran_no"] = $value;
	$showFields[] = "teguran_no";
		$showRawValues["teguran_no"] = substr($data["teguran_no"],0,100);
...
//	cetak_ke - 
	$value = $pageObject->showDBValue("cetak_ke", $data, $keylink);
	$showValues["cetak_ke"] = $value;
	$showFields[] = "cetak_ke";
		$showRawValues["cetak_ke"] = substr($data["cetak_ke"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_teguran";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_teguran_view.php?".$viewlink."'\"");
