...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_terima_line_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_terima_line_inline_edit.htm" : "pad_pad_terima_line_edit.htm";
...
//	processing terima_id - begin
	$condition = 1;

	if($condition)
	{
		$control_terima_id = $pageObject->getControl("terima_id", $id);
		$control_terima_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_terima_line_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	$data["terima_id"] = $evalues["terima_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("terima_id_label","<label for=\"".GetInputElementId("terima_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_terima_line_view.php?".$viewlink."'\"");
