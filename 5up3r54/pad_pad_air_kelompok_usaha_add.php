...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_kelompok_usaha_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_air_kelompok_usaha_inline_add.htm";
else
	$templatefile = "pad_pad_air_kelompok_usaha_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_air_kelompok_usaha_add.php');
...
//	processing induk_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_induk_id = $pageObject->getControl("induk_id", $id);
		$control_induk_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing id_old1 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_old1 = $pageObject->getControl("id_old1", $id);
		$control_id_old1->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing id_old1 - end
//	processing id_old2 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_old2 = $pageObject->getControl("id_old2", $id);
		$control_id_old2->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing id_old2 - end
//	processing id_old3 - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_id_old3 = $pageObject->getControl("id_old3", $id);
		$control_id_old3->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_air_kelompok_usaha_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_air_kelompok_usaha_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_air_kelompok_usaha_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["induk_id"]=@$avalues["induk_id"];
	...
	$defvalues["id_old1"]=@$avalues["id_old1"];
	$defvalues["id_old2"]=@$avalues["id_old2"];
	$defvalues["id_old3"]=@$avalues["id_old3"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("induk_id_label","<label for=\"".GetInputElementId("induk_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("id_old1_label","<label for=\"".GetInputElementId("id_old1", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("id_old2_label","<label for=\"".GetInputElementId("id_old2", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("id_old3_label","<label for=\"".GetInputElementId("id_old3", $id, PAGE_ADD)."\">","</label>");
