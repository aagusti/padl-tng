...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_kohir_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_kohir_inline_add.htm";
else
	$templatefile = "pad_pad_kohir_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_kohir_add.php');
...
//	processing kohirno - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_kohirno = $pageObject->getControl("kohirno", $id);
		$control_kohirno->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing kohirno - end
//	processing kohirtgl - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_kohirtgl = $pageObject->getControl("kohirtgl", $id);
		$control_kohirtgl->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_kohir_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_kohir_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_kohir_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["kohirno"]=@$avalues["kohirno"];
	$defvalues["kohirtgl"]=@$avalues["kohirtgl"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("kohirno_label","<label for=\"".GetInputElementId("kohirno", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kohirtgl_label","<label for=\"".GetInputElementId("kohirtgl", $id, PAGE_ADD)."\">","</label>");
