...
include("include/pad_pad_reklame_nsr_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_reklame_nsr_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_reklame_nsr_search.htm";
...
$params['shortTableName'] = 'pad_pad_reklame_nsr';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("njopr"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("njopr")] = GetTableURL($pageObject->pSet->getLookupTable("njopr"));
	
	$pageObject->fillFieldToolTips("njopr");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("njopr");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("njopr_label","<label for=\"".GetInputElementId("njopr", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("njopr_label", true);
	
	$xt->assign("njopr_fieldblock", true);
	$xt->assignbyref("njopr_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("njopr_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("njopr_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_njopr", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("njopr");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"njopr", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"njopr", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("nspr"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("nspr")] = GetTableURL($pageObject->pSet->getLookupTable("nspr"));
	
	$pageObject->fillFieldToolTips("nspr");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("nspr");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("nspr_label","<label for=\"".GetInputElementId("nspr", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("nspr_label", true);
	
	$xt->assign("nspr_fieldblock", true);
	$xt->assignbyref("nspr_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("nspr_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("nspr_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_nspr", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("nspr");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"nspr", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"nspr", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
