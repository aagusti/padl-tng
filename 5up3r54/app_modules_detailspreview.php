...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["app_modules_detailspreview"] = $layout;
...
if($mastertable == "app.apps")
{
	$where = "";
		$where .= GetFullFieldName("app_id", $strTableName, false)."=".make_db_value("app_id",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	app_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("app_id", $data, $keylink);
			$row["app_id_value"] = $value;
	//	kode - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("kode", $data, $keylink);
			$row["kode_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("app_modules_detailspreview.htm");
