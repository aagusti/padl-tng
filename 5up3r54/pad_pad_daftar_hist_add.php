...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_daftar_hist_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_daftar_hist_inline_add.htm";
else
	$templatefile = "pad_pad_daftar_hist_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_daftar_hist_add.php');
...
//	processing status_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_status_id = $pageObject->getControl("status_id", $id);
		$control_status_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing status_id - end
...
//	processing status_id - start
			...
			if($inlineAddOption)
			{
				$control_status_id->afterSuccessfulSave();
			}
//	processing status_id - end
//	processing create_date - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_daftar_hist_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_daftar_hist_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_daftar_hist_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_daftar_status")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["status_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	...
	$defvalues["status_id"]=@$avalues["status_id"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("status_id"))
		$xt->assign("status_id_fieldblock",true);
	else
		$xt->assign("status_id_tabfieldblock",true);
	$xt->assign("status_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("status_id_label","<label for=\"".GetInputElementId("status_id", $id, PAGE_ADD)."\">","</label>");
...
//	status_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("status_id", $data, $keylink);
		$showValues["status_id"] = $value;
		$showFields[] = "status_id";
	}	
//	create_date
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	keterangan
	...
	{	
		...
		$showRawValues["status_id"] = substr($data["status_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_daftar_hist";
		$options['firstTime'] = 1;
