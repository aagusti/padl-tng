...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_terima_export"] = $layout;
...
$xt->display("pad_pad_terima_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_terima.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_terima.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_terima.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["terimano"] = $pageObject->showDBValue("terimano", $row);
			$values["terimatgl"] = $pageObject->showDBValue("terimatgl", $row);
			$values["jmlterima"] = $pageObject->showDBValue("jmlterima", $row);
			$values["customer_id"] = $pageObject->showDBValue("customer_id", $row);
			$values["npwpd"] = $pageObject->showDBValue("npwpd", $row);
			$values["nobukti"] = $pageObject->showDBValue("nobukti", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_terima.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["jmlterima"] = $row["jmlterima"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["terimano"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["terimatgl"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["jmlterima"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nobukti"]).'"';
			if($outstr!="")
				$outstr.=",";
