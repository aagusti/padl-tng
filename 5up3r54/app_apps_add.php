...
$layout->blocks["top"][] = "details";$page_layouts["app_apps_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "app_apps_inline_add.htm";
else
	$templatefile = "app_apps_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: app_apps_add.php');
...
//	processing nama - end
//	processing app_path - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_app_path = $pageObject->getControl("app_path", $id);
		$control_app_path->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'app_apps_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'app_apps_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: app_apps_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["app_path"]=@$avalues["app_path"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("app_path_label","<label for=\"".GetInputElementId("app_path", $id, PAGE_ADD)."\">","</label>");
