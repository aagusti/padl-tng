...
include("include/public_tmp_bank2_null_sspd_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["public_tmp_bank2_null_sspd_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "public_tmp_bank2_null_sspd_search.htm";
...
$params['shortTableName'] = 'public_tmp_bank2_null_sspd';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("judul"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("judul")] = GetTableURL($pageObject->pSet->getLookupTable("judul"));
	
	$pageObject->fillFieldToolTips("judul");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("judul");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("judul_label","<label for=\"".GetInputElementId("judul", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("judul_label", true);
	
	$xt->assign("judul_fieldblock", true);
	$xt->assignbyref("judul_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("judul_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("judul_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_judul", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("judul");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"judul", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"judul", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("lokasi"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("lokasi")] = GetTableURL($pageObject->pSet->getLookupTable("lokasi"));
	
	$pageObject->fillFieldToolTips("lokasi");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("lokasi");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("lokasi_label","<label for=\"".GetInputElementId("lokasi", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("lokasi_label", true);
	
	$xt->assign("lokasi_fieldblock", true);
	$xt->assignbyref("lokasi_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("lokasi_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("lokasi_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_lokasi", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("lokasi");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"lokasi", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"lokasi", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("j"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("j")] = GetTableURL($pageObject->pSet->getLookupTable("j"));
	
	$pageObject->fillFieldToolTips("j");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("j");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("j_label","<label for=\"".GetInputElementId("j", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("j_label", true);
	
	$xt->assign("j_fieldblock", true);
	$xt->assignbyref("j_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("j_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("j_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_j", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("j");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"j", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"j", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("p"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("p")] = GetTableURL($pageObject->pSet->getLookupTable("p"));
	
	$pageObject->fillFieldToolTips("p");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("p");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("p_label","<label for=\"".GetInputElementId("p", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("p_label", true);
	
	$xt->assign("p_fieldblock", true);
	$xt->assignbyref("p_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("p_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("p_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_p", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("p");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"p", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"p", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("l"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("l")] = GetTableURL($pageObject->pSet->getLookupTable("l"));
	
	$pageObject->fillFieldToolTips("l");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("l");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("l_label","<label for=\"".GetInputElementId("l", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("l_label", true);
	
	$xt->assign("l_fieldblock", true);
	$xt->assignbyref("l_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("l_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("l_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_l", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("l");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"l", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"l", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("t"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("t")] = GetTableURL($pageObject->pSet->getLookupTable("t"));
	
	$pageObject->fillFieldToolTips("t");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("t");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("t_label","<label for=\"".GetInputElementId("t", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("t_label", true);
	
	$xt->assign("t_fieldblock", true);
	$xt->assignbyref("t_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("t_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("t_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_t", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("t");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"t", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"t", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
