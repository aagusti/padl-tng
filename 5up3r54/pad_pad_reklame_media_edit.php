...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_media_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_reklame_media_inline_edit.htm" : "pad_pad_reklame_media_edit.htm";
...
//	processing jenis_pajak_id - begin
	$condition = 1;

	if($condition)
	{
		$control_jenis_pajak_id = $pageObject->getControl("jenis_pajak_id", $id);
		$control_jenis_pajak_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jenis_pajak_id - end
//	processing media - begin
	$condition = 1;

	if($condition)
	{
		$control_media = $pageObject->getControl("media", $id);
		$control_media->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing media - end
//	processing singkatan - begin
	$condition = 1;

	if($condition)
	{
		$control_singkatan = $pageObject->getControl("singkatan", $id);
		$control_singkatan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing singkatan - end
//	processing masa - begin
	$condition = 1;

	if($condition)
	{
		$control_masa = $pageObject->getControl("masa", $id);
		$control_masa->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing jenis_pajak_id - begin
				...
				if($condition)
				{
					$control_jenis_pajak_id->afterSuccessfulSave();
				}
	//	processing jenis_pajak_id - end
			//	processing media - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_media->afterSuccessfulSave();
				}
	//	processing media - end
			//	processing singkatan - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_singkatan->afterSuccessfulSave();
				}
	//	processing singkatan - end
			//	processing masa - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_masa->afterSuccessfulSave();
				}
	//	processing masa - end
			//	processing keterangan - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_reklame_media_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_reklame_media_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["jenis_pajak_id"] = $evalues["jenis_pajak_id"];
	$data["media"] = $evalues["media"];
	$data["singkatan"] = $evalues["singkatan"];
	$data["masa"] = $evalues["masa"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("jenis_pajak_id"))
		$xt->assign("jenis_pajak_id_fieldblock",true);
	else
		$xt->assign("jenis_pajak_id_tabfieldblock",true);
	$xt->assign("jenis_pajak_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("jenis_pajak_id_label","<label for=\"".GetInputElementId("jenis_pajak_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("media"))
		$xt->assign("media_fieldblock",true);
	else
		$xt->assign("media_tabfieldblock",true);
	$xt->assign("media_label",true);
	if(isEnableSection508())
		$xt->assign_section("media_label","<label for=\"".GetInputElementId("media", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("singkatan"))
		$xt->assign("singkatan_fieldblock",true);
	else
		$xt->assign("singkatan_tabfieldblock",true);
	$xt->assign("singkatan_label",true);
	if(isEnableSection508())
		$xt->assign_section("singkatan_label","<label for=\"".GetInputElementId("singkatan", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("masa"))
		$xt->assign("masa_fieldblock",true);
	else
		$xt->assign("masa_tabfieldblock",true);
	$xt->assign("masa_label",true);
	if(isEnableSection508())
		$xt->assign_section("masa_label","<label for=\"".GetInputElementId("masa", $id, PAGE_EDIT)."\">","</label>");
...
//	jenis_pajak_id - 
	$value = $pageObject->showDBValue("jenis_pajak_id", $data, $keylink);
	$showValues["jenis_pajak_id"] = $value;
	$showFields[] = "jenis_pajak_id";
		$showRawValues["jenis_pajak_id"] = substr($data["jenis_pajak_id"],0,100);

//	media - 
	$value = $pageObject->showDBValue("media", $data, $keylink);
	$showValues["media"] = $value;
	$showFields[] = "media";
		$showRawValues["media"] = substr($data["media"],0,100);

//	singkatan - 
	$value = $pageObject->showDBValue("singkatan", $data, $keylink);
	$showValues["singkatan"] = $value;
	$showFields[] = "singkatan";
		$showRawValues["singkatan"] = substr($data["singkatan"],0,100);

//	masa - 
	$value = $pageObject->showDBValue("masa", $data, $keylink);
	$showValues["masa"] = $value;
	$showFields[] = "masa";
		$showRawValues["masa"] = substr($data["masa"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_reklame_media";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_reklame_media_view.php?".$viewlink."'\"");
