...
$layout->blocks["top"][] = "export";$page_layouts["pad_units_export"] = $layout;
...
$xt->display("pad_units_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_units.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_units.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_units.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["kategori"] = $pageObject->showDBValue("kategori", $row);
			$values["singkat"] = $pageObject->showDBValue("singkat", $row);
			$values["id"] = $pageObject->showDBValue("id", $row);
			$values["urusan_id"] = $pageObject->showDBValue("urusan_id", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_units.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["kategori"] = $pageObject->getViewControl("kategori")->showDBValue($row, "");
			$values["singkat"] = $pageObject->getViewControl("singkat")->showDBValue($row, "");
			$values["id"] = $pageObject->getViewControl("id")->showDBValue($row, "");
			$values["urusan_id"] = $pageObject->getViewControl("urusan_id")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["kategori"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["singkat"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["urusan_id"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Kategori").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Singkat").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Urusan Id").'</td>';	
	}
	else
	{
		echo "<td>"."Kode"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["kategori"]);
					else
						echo $values["kategori"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["singkat"]);
					else
						echo $values["singkat"];
			echo '</td>';
							echo '<td>';
