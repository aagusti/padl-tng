...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_air_zona_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_air_zona_search.htm";
...
$params['shortTableName'] = 'pad_pad_air_zona';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("indeks"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("indeks")] = GetTableURL($pageObject->pSet->getLookupTable("indeks"));
	
	$pageObject->fillFieldToolTips("indeks");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("indeks");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("indeks_label","<label for=\"".GetInputElementId("indeks", $id, PAGE_SEARCH)."\">","</label>");
	else 
	...
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("indeks");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"indeks", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"indeks", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
