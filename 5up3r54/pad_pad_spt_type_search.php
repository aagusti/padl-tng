...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_spt_type_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_spt_type_search.htm";
...
$params['shortTableName'] = 'pad_pad_spt_type';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("typenm"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("typenm")] = GetTableURL($pageObject->pSet->getLookupTable("typenm"));
	
	$pageObject->fillFieldToolTips("typenm");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("typenm");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("typenm_label","<label for=\"".GetInputElementId("typenm", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("typenm_label", true);
	
	$xt->assign("typenm_fieldblock", true);
	$xt->assignbyref("typenm_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("typenm_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("typenm_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_typenm", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("typenm");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"typenm", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"typenm", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
