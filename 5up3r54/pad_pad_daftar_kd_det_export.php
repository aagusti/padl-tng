...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_daftar_kd_det_export"] = $layout;
...
$xt->display("pad_pad_daftar_kd_det_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_daftar_kd_det.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_daftar_kd_det.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_daftar_kd_det.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["nourut"] = $pageObject->showDBValue("nourut", $row);
			$values["notes"] = $pageObject->showDBValue("notes", $row);
			$values["tarif"] = $pageObject->showDBValue("tarif", $row);
			$values["kamar"] = $pageObject->showDBValue("kamar", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_daftar_kd_det.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["nourut"] = $pageObject->getViewControl("nourut")->showDBValue($row, "");
			$values["notes"] = $pageObject->getViewControl("notes")->showDBValue($row, "");
			$values["tarif"] = $row["tarif"];
			$values["kamar"] = $pageObject->getViewControl("kamar")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["daftar_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nourut"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["notes"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["tarif"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["kamar"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nourut").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Notes").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tarif").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Kamar").'</td>';	
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["daftar_id"];//echo htmlspecialchars($values["daftar_id"]); commented for bug #6823
