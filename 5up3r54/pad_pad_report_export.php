...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_report_export"] = $layout;
...
$xt->display("pad_pad_report_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_report.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_report.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_report.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["spt_type_id"] = $pageObject->showDBValue("spt_type_id", $row);
			$values["usaha_id"] = $pageObject->showDBValue("usaha_id", $row);
			$values["sptnm"] = $pageObject->showDBValue("sptnm", $row);
			$values["sknm"] = $pageObject->showDBValue("sknm", $row);
			$values["nhnm"] = $pageObject->showDBValue("nhnm", $row);
			$values["sspdnm"] = $pageObject->showDBValue("sspdnm", $row);
			$values["stpnm"] = $pageObject->showDBValue("stpnm", $row);
			$values["kartudtnm"] = $pageObject->showDBValue("kartudtnm", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_report.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["spt_type_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["usaha_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["sptnm"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["sknm"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nhnm"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["sspdnm"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["stpnm"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["kartudtnm"]).'"';
			echo $outstr;
		}
