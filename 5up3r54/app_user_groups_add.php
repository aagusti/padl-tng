...
add_nocache_headers();
include("include/app_user_groups_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["app_user_groups_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "app_user_groups_inline_add.htm";
else
	$templatefile = "app_user_groups_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: app_user_groups_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing user_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_user_id = $pageObject->getControl("user_id", $id);
		$control_user_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing user_id - end
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="app.users")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["user_id"]==""){
			$avalues["user_id"] = prepare_for_db("user_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing user_id - start
			...
			if($inlineAddOption)
			{
				$control_user_id->afterSuccessfulSave();
			}
//	processing user_id - end
//	processing group_id - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'app_user_groups_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'app_user_groups_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: app_user_groups_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="app.users")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["user_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["user_id"]=@$avalues["user_id"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("user_id"))
		$xt->assign("user_id_fieldblock",true);
	else
		$xt->assign("user_id_tabfieldblock",true);
	$xt->assign("user_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("user_id_label","<label for=\"".GetInputElementId("user_id", $id, PAGE_ADD)."\">","</label>");
...
//	user_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("user_id", $data, $keylink);
		$showValues["user_id"] = $value;
		$showFields[] = "user_id";
	}	
//	group_id
	$display = false;
	...
	{	
		...
		$showRawValues["user_id"] = substr($data["user_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "app.user_groups";
		$options['firstTime'] = 1;
