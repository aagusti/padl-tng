...
$layout->blocks["top"][] = "details";$page_layouts["pad_units_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_units_inline_add.htm";
else
	$templatefile = "pad_units_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_units_add.php');
...
//	processing singkat - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_singkat = $pageObject->getControl("singkat", $id);
		$control_singkat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing singkat - end
//	processing urusan_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_urusan_id = $pageObject->getControl("urusan_id", $id);
		$control_urusan_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_units_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_units_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_units_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["singkat"]=@$avalues["singkat"];
	$defvalues["urusan_id"]=@$avalues["urusan_id"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("singkat_label","<label for=\"".GetInputElementId("singkat", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("urusan_id_label","<label for=\"".GetInputElementId("urusan_id", $id, PAGE_ADD)."\">","</label>");
