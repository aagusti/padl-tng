...
function GetTableURL($table = "")
{
	...
	if(!$table)
		$table=$strTableName;
	if("pad.pad_spt" == $table) 
		return "pad_pad_spt";
	if("pad.pad_spt_reklame" == $table) 
		return "pad_pad_spt_reklame";
	if("pad.pad_air_tanah_hit" == $table) 
		return "pad_pad_air_tanah_hit";
	if("pad.pad_air_tanah" == $table) 
		return "pad_pad_air_tanah";
	if("app.users" == $table) 
		return "app_users";
	if("public.pad_invoice" == $table) 
		return "public_pad_invoice";
	if("pad.pad_kohir" == $table) 
		return "pad_pad_kohir";
	if("pad.pad_customer_usaha" == $table) 
		return "pad_pad_customer_usaha";
	if("pad.pad_customer" == $table) 
		return "pad_pad_customer";
	if("app.app_status" == $table) 
		return "app_app_status";
	if("app.apps" == $table) 
		return "app_apps";
	if("app.group_modules" == $table) 
		return "app_group_modules";
	if("app.groups" == $table) 
		return "app_groups";
	if("app.modules" == $table) 
		return "app_modules";
	if("app.user_groups" == $table) 
		return "app_user_groups";
	if("pad.pad_air_hda" == $table) 
		return "pad_pad_air_hda";
	if("pad.pad_air_kelompok_usaha" == $table) 
		return "pad_pad_air_kelompok_usaha";
	if("pad.pad_air_manfaat" == $table) 
		return "pad_pad_air_manfaat";
	if("pad.pad_air_zona" == $table) 
		return "pad_pad_air_zona";
	if("pad.pad_anggaran" == $table) 
		return "pad_pad_anggaran";
	if("pad.pad_customer_detail" == $table) 
		return "pad_pad_customer_detail";
	if("pad.pad_customer_usaha_air_tanah" == $table) 
		return "pad_pad_customer_usaha_air_tanah";
	if("pad.pad_daftar" == $table) 
		return "pad_pad_daftar";
	if("pad.pad_daftar_hist" == $table) 
		return "pad_pad_daftar_hist";
	if("pad.pad_daftar_kd_det" == $table) 
		return "pad_pad_daftar_kd_det";
	if("pad.pad_daftar_status" == $table) 
		return "pad_pad_daftar_status";
	if("pad.pad_jenis_pajak" == $table) 
		return "pad_pad_jenis_pajak";
	if("pad.pad_kecamatan" == $table) 
		return "pad_pad_kecamatan";
	if("pad.pad_kelurahan" == $table) 
		return "pad_pad_kelurahan";
	if("pad.pad_pegawai" == $table) 
		return "pad_pad_pegawai";
	if("pad.pad_pemda" == $table) 
		return "pad_pad_pemda";
	if("pad.pad_periksa" == $table) 
		return "pad_pad_periksa";
	if("pad.pad_rekening" == $table) 
		return "pad_pad_rekening";
	if("pad.pad_reklame" == $table) 
		return "pad_pad_reklame";
	if("pad.pad_reklame_jalan" == $table) 
		return "pad_pad_reklame_jalan";
	if("pad.pad_reklame_kelas_jalan" == $table) 
		return "pad_pad_reklame_kelas_jalan";
	if("pad.pad_reklame_lokasi_pasang" == $table) 
		return "pad_pad_reklame_lokasi_pasang";
	if("pad.pad_reklame_mall" == $table) 
		return "pad_pad_reklame_mall";
	if("pad.pad_reklame_media" == $table) 
		return "pad_pad_reklame_media";
	if("pad.pad_reklame_nilai_strategis" == $table) 
		return "pad_pad_reklame_nilai_strategis";
	if("pad.pad_reklame_nsr" == $table) 
		return "pad_pad_reklame_nsr";
	if("pad.pad_reklame_sudut_pandang" == $table) 
		return "pad_pad_reklame_sudut_pandang";
	if("pad.pad_report" == $table) 
		return "pad_pad_report";
	if("pad.pad_spt_type" == $table) 
		return "pad_pad_spt_type";
	if("pad.pad_sspd" == $table) 
		return "pad_pad_sspd";
	if("pad.pad_stpd" == $table) 
		return "pad_pad_stpd";
	if("pad.pad_tarif_pajak" == $table) 
		return "pad_pad_tarif_pajak";
	if("pad.pad_teguran" == $table) 
		return "pad_pad_teguran";
	if("pad.pad_terima" == $table) 
		return "pad_pad_terima";
	if("pad.pad_terima_line" == $table) 
		return "pad_pad_terima_line";
	if("pad.pad_usaha" == $table) 
		return "pad_pad_usaha";
	if("pad.tmp_bayar" == $table) 
		return "pad_tmp_bayar";
	if("pad.units" == $table) 
		return "pad_units";
	if("pad.user_pad" == $table) 
		return "pad_user_pad";
	if("public.ar_payment_detail" == $table) 
		return "public_ar_payment_detail";
	if("public.pad_channel" == $table) 
		return "public_pad_channel";
	if("public.pad_payment" == $table) 
		return "public_pad_payment";
	if("public.pad_reversal" == $table) 
		return "public_pad_reversal";
	if("public.pad_sspd_bak" == $table) 
		return "public_pad_sspd_bak";
	if("public.tblpejabat2" == $table) 
		return "public_tblpejabat2";
	if("public.pad_tp" == $table) 
		return "public_pad_tp";
	if("public.tmp_bank" == $table) 
		return "public_tmp_bank";
	if("public.tmp_bank_2013" == $table) 
		return "public_tmp_bank_2013";
	if("public.tmp_bank_ok_1" == $table) 
		return "public_tmp_bank_ok_1";
	if("public.tmp_bank2" == $table) 
		return "public_tmp_bank2";
	if("public.tmp_bank2_null_sspd" == $table) 
		return "public_tmp_bank2_null_sspd";
}
...
{	
	...
	if(!$shortTName)
		return false;
	if("pad_pad_spt" == $shortTName) 
		return "pad.pad_spt";
	if("pad_pad_spt_reklame" == $shortTName) 
		return "pad.pad_spt_reklame";
	if("pad_pad_air_tanah_hit" == $shortTName) 
		return "pad.pad_air_tanah_hit";
	if("pad_pad_air_tanah" == $shortTName) 
		return "pad.pad_air_tanah";
	if("app_users" == $shortTName) 
		return "app.users";
	if("public_pad_invoice" == $shortTName) 
		return "public.pad_invoice";
	if("pad_pad_kohir" == $shortTName) 
		return "pad.pad_kohir";
	if("pad_pad_customer_usaha" == $shortTName) 
		return "pad.pad_customer_usaha";
	if("pad_pad_customer" == $shortTName) 
		return "pad.pad_customer";
	if("app_app_status" == $shortTName) 
		return "app.app_status";
	if("app_apps" == $shortTName) 
		return "app.apps";
	if("app_group_modules" == $shortTName) 
		return "app.group_modules";
	if("app_groups" == $shortTName) 
		return "app.groups";
	if("app_modules" == $shortTName) 
		return "app.modules";
	if("app_user_groups" == $shortTName) 
		return "app.user_groups";
	if("pad_pad_air_hda" == $shortTName) 
		return "pad.pad_air_hda";
	if("pad_pad_air_kelompok_usaha" == $shortTName) 
		return "pad.pad_air_kelompok_usaha";
	if("pad_pad_air_manfaat" == $shortTName) 
		return "pad.pad_air_manfaat";
	if("pad_pad_air_zona" == $shortTName) 
		return "pad.pad_air_zona";
	if("pad_pad_anggaran" == $shortTName) 
		return "pad.pad_anggaran";
	if("pad_pad_customer_detail" == $shortTName) 
		return "pad.pad_customer_detail";
	if("pad_pad_customer_usaha_air_tanah" == $shortTName) 
		return "pad.pad_customer_usaha_air_tanah";
	if("pad_pad_daftar" == $shortTName) 
		return "pad.pad_daftar";
	if("pad_pad_daftar_hist" == $shortTName) 
		return "pad.pad_daftar_hist";
	if("pad_pad_daftar_kd_det" == $shortTName) 
		return "pad.pad_daftar_kd_det";
	if("pad_pad_daftar_status" == $shortTName) 
		return "pad.pad_daftar_status";
	if("pad_pad_jenis_pajak" == $shortTName) 
		return "pad.pad_jenis_pajak";
	if("pad_pad_kecamatan" == $shortTName) 
		return "pad.pad_kecamatan";
	if("pad_pad_kelurahan" == $shortTName) 
		return "pad.pad_kelurahan";
	if("pad_pad_pegawai" == $shortTName) 
		return "pad.pad_pegawai";
	if("pad_pad_pemda" == $shortTName) 
		return "pad.pad_pemda";
	if("pad_pad_periksa" == $shortTName) 
		return "pad.pad_periksa";
	if("pad_pad_rekening" == $shortTName) 
		return "pad.pad_rekening";
	if("pad_pad_reklame" == $shortTName) 
		return "pad.pad_reklame";
	if("pad_pad_reklame_jalan" == $shortTName) 
		return "pad.pad_reklame_jalan";
	if("pad_pad_reklame_kelas_jalan" == $shortTName) 
		return "pad.pad_reklame_kelas_jalan";
	if("pad_pad_reklame_lokasi_pasang" == $shortTName) 
		return "pad.pad_reklame_lokasi_pasang";
	if("pad_pad_reklame_mall" == $shortTName) 
		return "pad.pad_reklame_mall";
	if("pad_pad_reklame_media" == $shortTName) 
		return "pad.pad_reklame_media";
	if("pad_pad_reklame_nilai_strategis" == $shortTName) 
		return "pad.pad_reklame_nilai_strategis";
	if("pad_pad_reklame_nsr" == $shortTName) 
		return "pad.pad_reklame_nsr";
	if("pad_pad_reklame_sudut_pandang" == $shortTName) 
		return "pad.pad_reklame_sudut_pandang";
	if("pad_pad_report" == $shortTName) 
		return "pad.pad_report";
	if("pad_pad_spt_type" == $shortTName) 
		return "pad.pad_spt_type";
	if("pad_pad_sspd" == $shortTName) 
		return "pad.pad_sspd";
	if("pad_pad_stpd" == $shortTName) 
		return "pad.pad_stpd";
	if("pad_pad_tarif_pajak" == $shortTName) 
		return "pad.pad_tarif_pajak";
	if("pad_pad_teguran" == $shortTName) 
		return "pad.pad_teguran";
	if("pad_pad_terima" == $shortTName) 
		return "pad.pad_terima";
	if("pad_pad_terima_line" == $shortTName) 
		return "pad.pad_terima_line";
	if("pad_pad_usaha" == $shortTName) 
		return "pad.pad_usaha";
	if("pad_tmp_bayar" == $shortTName) 
		return "pad.tmp_bayar";
	if("pad_units" == $shortTName) 
		return "pad.units";
	if("pad_user_pad" == $shortTName) 
		return "pad.user_pad";
	if("public_ar_payment_detail" == $shortTName) 
		return "public.ar_payment_detail";
	if("public_pad_channel" == $shortTName) 
		return "public.pad_channel";
	if("public_pad_payment" == $shortTName) 
		return "public.pad_payment";
	if("public_pad_reversal" == $shortTName) 
		return "public.pad_reversal";
	if("public_pad_sspd_bak" == $shortTName) 
		return "public.pad_sspd_bak";
	if("public_tblpejabat2" == $shortTName) 
		return "public.tblpejabat2";
	if("public_pad_tp" == $shortTName) 
		return "public.pad_tp";
	if("public_tmp_bank" == $shortTName) 
		return "public.tmp_bank";
	if("public_tmp_bank_2013" == $shortTName) 
		return "public.tmp_bank_2013";
	if("public_tmp_bank_ok_1" == $shortTName) 
		return "public.tmp_bank_ok_1";
	if("public_tmp_bank2" == $shortTName) 
		return "public.tmp_bank2";
	if("public_tmp_bank2_null_sspd" == $shortTName) 
		return "public.tmp_bank2_null_sspd";
}
