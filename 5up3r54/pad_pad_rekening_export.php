...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_rekening_export"] = $layout;
...
$xt->display("pad_pad_rekening_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_rekening.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_rekening.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_rekening.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["levelid"] = $pageObject->showDBValue("levelid", $row);
			$values["issummary"] = $pageObject->showDBValue("issummary", $row);
			$values["defsign"] = $pageObject->showDBValue("defsign", $row);
			$values["isppkd"] = $pageObject->showDBValue("isppkd", $row);
			...
			$values["insidentil"] = $pageObject->showDBValue("insidentil", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_rekening.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["levelid"] = $pageObject->getViewControl("levelid")->showDBValue($row, "");
			$values["issummary"] = $pageObject->getViewControl("issummary")->showDBValue($row, "");
			$values["defsign"] = $pageObject->getViewControl("defsign")->showDBValue($row, "");
			$values["isppkd"] = $pageObject->getViewControl("isppkd")->showDBValue($row, "");
			...
			$values["insidentil"] = $pageObject->getViewControl("insidentil")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["levelid"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["issummary"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["defsign"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["isppkd"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["insidentil"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Levelid").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Issummary").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Defsign").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Isppkd").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Insidentil").'</td>';	
	}
	else
	{
		echo "<td>"."Id"."</td>";
