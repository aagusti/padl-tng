...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_tarif_pajak_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_tarif_pajak_inline_edit.htm" : "pad_pad_tarif_pajak_edit.htm";
...
//	processing reklame - begin
	$condition = 1;

	if($condition)
	{
		$control_reklame = $pageObject->getControl("reklame", $id);
		$control_reklame->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing reklame - end
...
//	processing update_uid - end
	...
	//	processing tarif - end
			//	processing reklame - begin
				...
				if($condition)
				{
					$control_reklame->afterSuccessfulSave();
				}
	//	processing reklame - end
			//	processing minimal_omset - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_tarif_pajak_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_tarif_pajak_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("reklame"))
		$xt->assign("reklame_fieldblock",true);
	else
		$xt->assign("reklame_tabfieldblock",true);
	$xt->assign("reklame_label",true);
	if(isEnableSection508())
		$xt->assign_section("reklame_label","<label for=\"".GetInputElementId("reklame", $id, PAGE_EDIT)."\">","</label>");
...
//	reklame - Number
	$value = $pageObject->showDBValue("reklame", $data, $keylink);
	$showValues["reklame"] = $value;
	$showFields[] = "reklame";
		$showRawValues["reklame"] = substr($data["reklame"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_tarif_pajak";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_tarif_pajak_view.php?".$viewlink."'\"");
