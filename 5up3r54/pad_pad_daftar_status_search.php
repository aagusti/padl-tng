...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_daftar_status_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_daftar_status_search.htm";
...
$params['shortTableName'] = 'pad_pad_daftar_status';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("uraian"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("uraian")] = GetTableURL($pageObject->pSet->getLookupTable("uraian"));
	
	$pageObject->fillFieldToolTips("uraian");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("uraian");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("uraian_label","<label for=\"".GetInputElementId("uraian", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("uraian_label", true);
	
	$xt->assign("uraian_fieldblock", true);
	$xt->assignbyref("uraian_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("uraian_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("uraian_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_uraian", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("uraian");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"uraian", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"uraian", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
