...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_tarif_pajak_export"] = $layout;
...
$xt->display("pad_pad_tarif_pajak_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_tarif_pajak.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_tarif_pajak.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_tarif_pajak.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["reklame"] = $pageObject->showDBValue("reklame", $row);
			$values["minimal_omset"] = $pageObject->showDBValue("minimal_omset", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_tarif_pajak.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["reklame"] = $row["reklame"];
			$values["minimal_omset"] = $row["minimal_omset"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["reklame"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["minimal_omset"]).'"';
			if($outstr!="")
				$outstr.=",";
