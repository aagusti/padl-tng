...
$layout->blocks["top"][] = "details";$page_layouts["public_pad_reversal_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "public_pad_reversal_inline_edit.htm" : "public_pad_reversal_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: public_pad_reversal_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: public_pad_reversal_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "public.pad_reversal";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='public_pad_reversal_view.php?".$viewlink."'\"");
