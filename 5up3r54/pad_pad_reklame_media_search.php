...
include("include/pad_pad_reklame_media_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_reklame_media_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_reklame_media_search.htm";
...
$params['shortTableName'] = 'pad_pad_reklame_media';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("jenis_pajak_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("jenis_pajak_id")] = GetTableURL($pageObject->pSet->getLookupTable("jenis_pajak_id"));
	
	$pageObject->fillFieldToolTips("jenis_pajak_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("jenis_pajak_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "jenis_pajak_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("jenis_pajak_id_label","<label for=\"".GetInputElementId("jenis_pajak_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("jenis_pajak_id_label", true);
	
	$xt->assign("jenis_pajak_id_fieldblock", true);
	$xt->assignbyref("jenis_pajak_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("jenis_pajak_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("jenis_pajak_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_jenis_pajak_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("jenis_pajak_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"jenis_pajak_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"jenis_pajak_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("media"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("media")] = GetTableURL($pageObject->pSet->getLookupTable("media"));
	
	$pageObject->fillFieldToolTips("media");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("media");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("media_label","<label for=\"".GetInputElementId("media", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("media_label", true);
	
	$xt->assign("media_fieldblock", true);
	$xt->assignbyref("media_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("media_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("media_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_media", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("media");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"media", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"media", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("singkatan"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("singkatan")] = GetTableURL($pageObject->pSet->getLookupTable("singkatan"));
	
	$pageObject->fillFieldToolTips("singkatan");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("singkatan");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "singkatan";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("singkatan_label","<label for=\"".GetInputElementId("singkatan", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("singkatan_label", true);
	
	$xt->assign("singkatan_fieldblock", true);
	$xt->assignbyref("singkatan_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("singkatan_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("singkatan_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_singkatan", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("singkatan");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"singkatan", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"singkatan", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("masa"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("masa")] = GetTableURL($pageObject->pSet->getLookupTable("masa"));
	
	$pageObject->fillFieldToolTips("masa");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("masa");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("masa_label","<label for=\"".GetInputElementId("masa", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("masa_label", true);
	
	$xt->assign("masa_fieldblock", true);
	$xt->assignbyref("masa_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("masa_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("masa_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_masa", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("masa");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"masa", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"masa", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
