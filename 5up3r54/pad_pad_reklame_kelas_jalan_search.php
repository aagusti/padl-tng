...
include("include/pad_pad_reklame_kelas_jalan_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_reklame_kelas_jalan_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_reklame_kelas_jalan_search.htm";
...
$params['shortTableName'] = 'pad_pad_reklame_kelas_jalan';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("nilai"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("nilai")] = GetTableURL($pageObject->pSet->getLookupTable("nilai"));
	
	$pageObject->fillFieldToolTips("nilai");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("nilai");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("nilai_label","<label for=\"".GetInputElementId("nilai", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("nilai_label", true);
	
	$xt->assign("nilai_fieldblock", true);
	$xt->assignbyref("nilai_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("nilai_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("nilai_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_nilai", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("nilai");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"nilai", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"nilai", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("kriteria"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("kriteria")] = GetTableURL($pageObject->pSet->getLookupTable("kriteria"));
	
	$pageObject->fillFieldToolTips("kriteria");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("kriteria");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "kriteria";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("kriteria_label","<label for=\"".GetInputElementId("kriteria", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("kriteria_label", true);
	
	$xt->assign("kriteria_fieldblock", true);
	$xt->assignbyref("kriteria_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("kriteria_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("kriteria_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_kriteria", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("kriteria");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"kriteria", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"kriteria", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
