...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_periksa_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_periksa_inline_add.htm";
else
	$templatefile = "pad_pad_periksa_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_periksa_add.php');
...
//	processing periksa_no - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_periksa_no = $pageObject->getControl("periksa_no", $id);
		$control_periksa_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing periksa_no - end
//	processing periksa_tgl - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_periksa_tgl = $pageObject->getControl("periksa_tgl", $id);
		$control_periksa_tgl->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing lhp_no - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_lhp_no = $pageObject->getControl("lhp_no", $id);
		$control_lhp_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_periksa_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_periksa_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_periksa_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["periksa_no"]=@$avalues["periksa_no"];
	$defvalues["periksa_tgl"]=@$avalues["periksa_tgl"];
	$defvalues["invoice_id"]=@$avalues["invoice_id"];
	$defvalues["lhp_no"]=@$avalues["lhp_no"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("periksa_no_label","<label for=\"".GetInputElementId("periksa_no", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("periksa_tgl_label","<label for=\"".GetInputElementId("periksa_tgl", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("lhp_no_label","<label for=\"".GetInputElementId("lhp_no", $id, PAGE_ADD)."\">","</label>");
