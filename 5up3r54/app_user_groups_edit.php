...
$layout->blocks["top"][] = "details";$page_layouts["app_user_groups_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "app_user_groups_inline_edit.htm" : "app_user_groups_edit.htm";
...
//	processing user_id - begin
	$condition = 1;

	if($condition)
	{
		$control_user_id = $pageObject->getControl("user_id", $id);
		$control_user_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: app_user_groups_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: app_user_groups_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("user_id_label","<label for=\"".GetInputElementId("user_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='app_user_groups_view.php?".$viewlink."'\"");
