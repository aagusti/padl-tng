...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_daftar_hist_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_daftar_hist_inline_edit.htm" : "pad_pad_daftar_hist_edit.htm";
...
//	processing status_id - begin
	$condition = 1;

	if($condition)
	{
		$control_status_id = $pageObject->getControl("status_id", $id);
		$control_status_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_daftar_hist_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_daftar_hist_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["status_id"] = $evalues["status_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("status_id_label","<label for=\"".GetInputElementId("status_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_daftar_hist_view.php?".$viewlink."'\"");
