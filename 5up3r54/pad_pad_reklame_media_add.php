...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_media_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_reklame_media_inline_add.htm";
else
	$templatefile = "pad_pad_reklame_media_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_reklame_media_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing jenis_pajak_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jenis_pajak_id = $pageObject->getControl("jenis_pajak_id", $id);
		$control_jenis_pajak_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing media - start
	...
	if($inlineAddOption)
	{
		...
		$control_media->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing media - end
//	processing singkatan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_singkatan = $pageObject->getControl("singkatan", $id);
		$control_singkatan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing singkatan - end
//	processing masa - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_masa = $pageObject->getControl("masa", $id);
		$control_masa->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing masa - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_media_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_media_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_reklame_media_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["jenis_pajak_id"]=@$avalues["jenis_pajak_id"];
	$defvalues["media"]=@$avalues["media"];
	$defvalues["singkatan"]=@$avalues["singkatan"];
	$defvalues["masa"]=@$avalues["masa"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("jenis_pajak_id_label","<label for=\"".GetInputElementId("jenis_pajak_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("media_label","<label for=\"".GetInputElementId("media", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("singkatan_label","<label for=\"".GetInputElementId("singkatan", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("masa_label","<label for=\"".GetInputElementId("masa", $id, PAGE_ADD)."\">","</label>");
