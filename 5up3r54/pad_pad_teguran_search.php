...
include("include/pad_pad_teguran_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_teguran_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_teguran_search.htm";
...
$params['shortTableName'] = 'pad_pad_teguran';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("teguran_no"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("teguran_no")] = GetTableURL($pageObject->pSet->getLookupTable("teguran_no"));
	
	$pageObject->fillFieldToolTips("teguran_no");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("teguran_no");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "teguran_no";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("teguran_no_label","<label for=\"".GetInputElementId("teguran_no", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("teguran_no_label", true);
	
	$xt->assign("teguran_no_fieldblock", true);
	$xt->assignbyref("teguran_no_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("teguran_no_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("teguran_no_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_teguran_no", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("teguran_no");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"teguran_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"teguran_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("cetak_ke"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("cetak_ke")] = GetTableURL($pageObject->pSet->getLookupTable("cetak_ke"));
	
	$pageObject->fillFieldToolTips("cetak_ke");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("cetak_ke");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "cetak_ke";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("cetak_ke_label","<label for=\"".GetInputElementId("cetak_ke", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("cetak_ke_label", true);
	
	$xt->assign("cetak_ke_fieldblock", true);
	$xt->assignbyref("cetak_ke_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("cetak_ke_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("cetak_ke_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_cetak_ke", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("cetak_ke");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"cetak_ke", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"cetak_ke", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
