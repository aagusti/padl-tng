...
$layout->blocks["top"][] = "details";$page_layouts["app_apps_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "app_apps_inline_edit.htm" : "app_apps_edit.htm";
...
//	processing nama - end
//	processing app_path - begin
	$condition = 1;

	if($condition)
	{
		$control_app_path = $pageObject->getControl("app_path", $id);
		$control_app_path->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing app_path - end
...
//	processing disabled - end
	...
	//	processing nama - end
			//	processing app_path - begin
				...
				if($condition)
				{
					$control_app_path->afterSuccessfulSave();
				}
	//	processing app_path - end
			//	processing disabled - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: app_apps_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: app_apps_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["app_path"] = $evalues["app_path"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("app_path"))
		$xt->assign("app_path_fieldblock",true);
	else
		$xt->assign("app_path_tabfieldblock",true);
	$xt->assign("app_path_label",true);
	if(isEnableSection508())
		$xt->assign_section("app_path_label","<label for=\"".GetInputElementId("app_path", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	$showDetailKeys["app.modules"]["masterkey1"] = $data["id"];		
...
//	app_path - 
	$value = $pageObject->showDBValue("app_path", $data, $keylink);
	$showValues["app_path"] = $value;
	$showFields[] = "app_path";
		$showRawValues["app_path"] = substr($data["app_path"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "app.apps";
		$options['firstTime'] = 1;
		
		$strTableName = $dpParams['strTableNames'][$d];
		
		if(!CheckSecurity(@$_SESSION["_".$strTableName."_OwnerID"],"Search")){
			$strTableName = "app.apps";
			continue;
		}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='app_apps_view.php?".$viewlink."'\"");
