...
$layout->blocks["top"][] = "details";$page_layouts["pad_tmp_bayar_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_tmp_bayar_inline_add.htm";
else
	$templatefile = "pad_tmp_bayar_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_tmp_bayar_add.php');
...
//	processing old_spt_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_old_spt_id = $pageObject->getControl("old_spt_id", $id);
		$control_old_spt_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing wp - start
	...
	if($inlineAddOption)
	{
		...
		$control_wp->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing wp - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_tmp_bayar_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_tmp_bayar_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_tmp_bayar_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["old_spt_id"]=@$avalues["old_spt_id"];
	$defvalues["wp"]=@$avalues["wp"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("old_spt_id_label","<label for=\"".GetInputElementId("old_spt_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("wp_label","<label for=\"".GetInputElementId("wp", $id, PAGE_ADD)."\">","</label>");
