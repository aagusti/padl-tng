...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_spt_type_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_spt_type_inline_add.htm";
else
	$templatefile = "pad_pad_spt_type_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_spt_type_add.php');
...
//	processing id - end
//	processing typenm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_typenm = $pageObject->getControl("typenm", $id);
		$control_typenm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_spt_type_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_spt_type_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_spt_type_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["typenm"]=@$avalues["typenm"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("typenm_label","<label for=\"".GetInputElementId("typenm", $id, PAGE_ADD)."\">","</label>");
