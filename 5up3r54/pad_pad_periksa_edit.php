...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_periksa_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_periksa_inline_edit.htm" : "pad_pad_periksa_edit.htm";
...
//	processing periksa_no - begin
	$condition = 1;

	if($condition)
	{
		$control_periksa_no = $pageObject->getControl("periksa_no", $id);
		$control_periksa_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing periksa_no - end
//	processing periksa_tgl - begin
	$condition = 1;

	if($condition)
	{
		$control_periksa_tgl = $pageObject->getControl("periksa_tgl", $id);
		$control_periksa_tgl->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing periksa_tgl - end
...
//	processing lhp_no - begin
	$condition = 1;

	if($condition)
	{
		$control_lhp_no = $pageObject->getControl("lhp_no", $id);
		$control_lhp_no->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing lhp_no - end
...
//	processing update_uid - end
	...
	//	processing tahun - end
			//	processing periksa_no - begin
				...
				if($condition)
				{
					$control_periksa_no->afterSuccessfulSave();
				}
	//	processing periksa_no - end
			//	processing periksa_tgl - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_periksa_tgl->afterSuccessfulSave();
				}
	//	processing periksa_tgl - end
			//	processing invoice_id - begin
							$condition = 1;
	...
	//	processing invoice_id - end
			//	processing lhp_no - begin
				...
				if($condition)
				{
					$control_lhp_no->afterSuccessfulSave();
				}
	//	processing lhp_no - end
			//	processing keterangan - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_periksa_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["periksa_no"] = $evalues["periksa_no"];
	$data["periksa_tgl"] = $evalues["periksa_tgl"];
	$data["invoice_id"] = $evalues["invoice_id"];
	$data["lhp_no"] = $evalues["lhp_no"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("periksa_no_label","<label for=\"".GetInputElementId("periksa_no", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("periksa_tgl_label","<label for=\"".GetInputElementId("periksa_tgl", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("lhp_no_label","<label for=\"".GetInputElementId("lhp_no", $id, PAGE_EDIT)."\">","</label>");
...
//	periksa_no - 
	...
	$showValues["periksa_no"] = $value;
	$showFields[] = "periksa_no";
		$showRawValues["periksa_no"] = substr($data["periksa_no"],0,100);

//	periksa_tgl - Short Date
	$value = $pageObject->showDBValue("periksa_tgl", $data, $keylink);
	$showValues["periksa_tgl"] = $value;
	$showFields[] = "periksa_tgl";
		$showRawValues["periksa_tgl"] = substr($data["periksa_tgl"],0,100);
...
//	lhp_no - 
	...
	$showValues["lhp_no"] = $value;
	$showFields[] = "lhp_no";
		$showRawValues["lhp_no"] = substr($data["lhp_no"],0,100);
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_periksa_view.php?".$viewlink."'\"");
