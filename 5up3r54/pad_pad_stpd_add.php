...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_stpd_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_stpd_inline_add.htm";
else
	$templatefile = "pad_pad_stpd_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_stpd_add.php');
...
//	processing stpdno - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_stpdno = $pageObject->getControl("stpdno", $id);
		$control_stpdno->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing stpdno - end
//	processing stpdtgl - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_stpdtgl = $pageObject->getControl("stpdtgl", $id);
		$control_stpdtgl->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing stpdtgl - end
...
//	processing stpdcount - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_stpdcount = $pageObject->getControl("stpdcount", $id);
		$control_stpdcount->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing stpdcount - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_spt")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["spt_id"]==""){
			$avalues["spt_id"] = prepare_for_db("spt_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing stpdno - start
			...
			if($inlineAddOption)
			{
				$control_stpdno->afterSuccessfulSave();
			}
//	processing stpdno - end
//	processing stpdtgl - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_stpdtgl->afterSuccessfulSave();
			}
//	processing stpdtgl - end
//	processing spt_id - start
			$inlineAddOption = true;
...
//	processing stpdcount - start
			...
			if($inlineAddOption)
			{
				$control_stpdcount->afterSuccessfulSave();
			}
//	processing stpdcount - end
//	processing bunga - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_stpd_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_stpd_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_stpd_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_spt")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["spt_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("stpdno"))
		$xt->assign("stpdno_fieldblock",true);
	else
		$xt->assign("stpdno_tabfieldblock",true);
	$xt->assign("stpdno_label",true);
	if(isEnableSection508())
		$xt->assign_section("stpdno_label","<label for=\"".GetInputElementId("stpdno", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("stpdtgl"))
		$xt->assign("stpdtgl_fieldblock",true);
	else
		$xt->assign("stpdtgl_tabfieldblock",true);
	$xt->assign("stpdtgl_label",true);
	if(isEnableSection508())
		$xt->assign_section("stpdtgl_label","<label for=\"".GetInputElementId("stpdtgl", $id, PAGE_ADD)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("stpdcount"))
		$xt->assign("stpdcount_fieldblock",true);
	else
		$xt->assign("stpdcount_tabfieldblock",true);
	$xt->assign("stpdcount_label",true);
	if(isEnableSection508())
		$xt->assign_section("stpdcount_label","<label for=\"".GetInputElementId("stpdcount", $id, PAGE_ADD)."\">","</label>");
...
//	stpdno
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("stpdno", $data, $keylink);
		$showValues["stpdno"] = $value;
		$showFields[] = "stpdno";
	}	
//	stpdtgl
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("stpdtgl", $data, $keylink);
		$showValues["stpdtgl"] = $value;
		$showFields[] = "stpdtgl";
	}	
//	spt_id
	$display = false;
...
//	stpdcount
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("stpdcount", $data, $keylink);
		$showValues["stpdcount"] = $value;
		$showFields[] = "stpdcount";
	}	
//	bunga
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	pejabat_id
	...
	{	
		...
		$showRawValues["stpdno"] = substr($data["stpdno"],0,100);
		$showRawValues["stpdtgl"] = substr($data["stpdtgl"],0,100);
		...
		$showRawValues["stpdcount"] = substr($data["stpdcount"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_stpd";
		$options['firstTime'] = 1;
