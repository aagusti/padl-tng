...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_terima_line_add"] = $layout;
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_terima_line_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing terima_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_terima_id = $pageObject->getControl("terima_id", $id);
		$control_terima_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing terima_id - end
...
//	processing terima_id - start
			...
			if($inlineAddOption)
			{
				$control_terima_id->afterSuccessfulSave();
			}
//	processing terima_id - end
//	processing spt_id - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_terima_line_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_terima_line_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_terima_line_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["terima_id"]=@$avalues["terima_id"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("terima_id"))
		$xt->assign("terima_id_fieldblock",true);
	else
		$xt->assign("terima_id_tabfieldblock",true);
	$xt->assign("terima_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("terima_id_label","<label for=\"".GetInputElementId("terima_id", $id, PAGE_ADD)."\">","</label>");
...
//	terima_id
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("terima_id", $data, $keylink);
		$showValues["terima_id"] = $value;
		$showFields[] = "terima_id";
	}	
//	spt_id
	$display = false;
	if($inlineadd==ADD_MASTER)
...
//	write_uid
	...
	{	
		...
		$showRawValues["terima_id"] = substr($data["terima_id"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_terima_line";
		$options['firstTime'] = 1;
