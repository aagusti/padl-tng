...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_jenis_pajak_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_jenis_pajak_inline_edit.htm" : "pad_pad_jenis_pajak_edit.htm";
...
//	processing rekening_kd_sub - begin
	$condition = 1;

	if($condition)
	{
		$control_rekening_kd_sub = $pageObject->getControl("rekening_kd_sub", $id);
		$control_rekening_kd_sub->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing rekening_kd_sub - end
//	processing rekdenda_id - begin
	$condition = 1;

	if($condition)
	{
		$control_rekdenda_id = $pageObject->getControl("rekdenda_id", $id);
		$control_rekdenda_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing rekdenda_id - end
//	processing masapajak - begin
	$condition = 1;

	if($condition)
	{
		$control_masapajak = $pageObject->getControl("masapajak", $id);
		$control_masapajak->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing masapajak - end
//	processing jatuhtempo - begin
	$condition = 1;

	if($condition)
	{
		$control_jatuhtempo = $pageObject->getControl("jatuhtempo", $id);
		$control_jatuhtempo->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jatuhtempo - end
...
//	processing jalan_kelas_id - begin
	$condition = 1;

	if($condition)
	{
		$control_jalan_kelas_id = $pageObject->getControl("jalan_kelas_id", $id);
		$control_jalan_kelas_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jalan_kelas_id - end
...
//	processing update_uid - end
	...
	//	processing rekening_id - end
			//	processing rekening_kd_sub - begin
				...
				if($condition)
				{
					$control_rekening_kd_sub->afterSuccessfulSave();
				}
	//	processing rekening_kd_sub - end
			//	processing rekdenda_id - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_rekdenda_id->afterSuccessfulSave();
				}
	//	processing rekdenda_id - end
			//	processing masapajak - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_masapajak->afterSuccessfulSave();
				}
	//	processing masapajak - end
			//	processing jatuhtempo - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_jatuhtempo->afterSuccessfulSave();
				}
	//	processing jatuhtempo - end
			//	processing multiple - begin
							$condition = 1;
	...
	//	processing multiple - end
			//	processing jalan_kelas_id - begin
				...
				if($condition)
				{
					$control_jalan_kelas_id->afterSuccessfulSave();
				}
	//	processing jalan_kelas_id - end
			//	processing tmt - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_jenis_pajak_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_jenis_pajak_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["rekening_kd_sub"] = $evalues["rekening_kd_sub"];
	$data["rekdenda_id"] = $evalues["rekdenda_id"];
	$data["masapajak"] = $evalues["masapajak"];
	$data["jatuhtempo"] = $evalues["jatuhtempo"];
	$data["multiple"] = $evalues["multiple"];
	$data["jalan_kelas_id"] = $evalues["jalan_kelas_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("rekening_kd_sub"))
		$xt->assign("rekening_kd_sub_fieldblock",true);
	else
		$xt->assign("rekening_kd_sub_tabfieldblock",true);
	$xt->assign("rekening_kd_sub_label",true);
	if(isEnableSection508())
		$xt->assign_section("rekening_kd_sub_label","<label for=\"".GetInputElementId("rekening_kd_sub", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("rekdenda_id"))
		$xt->assign("rekdenda_id_fieldblock",true);
	else
		$xt->assign("rekdenda_id_tabfieldblock",true);
	$xt->assign("rekdenda_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("rekdenda_id_label","<label for=\"".GetInputElementId("rekdenda_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("masapajak"))
		$xt->assign("masapajak_fieldblock",true);
	else
		$xt->assign("masapajak_tabfieldblock",true);
	$xt->assign("masapajak_label",true);
	if(isEnableSection508())
		$xt->assign_section("masapajak_label","<label for=\"".GetInputElementId("masapajak", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("jatuhtempo"))
		$xt->assign("jatuhtempo_fieldblock",true);
	else
		$xt->assign("jatuhtempo_tabfieldblock",true);
	$xt->assign("jatuhtempo_label",true);
	if(isEnableSection508())
		$xt->assign_section("jatuhtempo_label","<label for=\"".GetInputElementId("jatuhtempo", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("jalan_kelas_id"))
		$xt->assign("jalan_kelas_id_fieldblock",true);
	else
		$xt->assign("jalan_kelas_id_tabfieldblock",true);
	$xt->assign("jalan_kelas_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("jalan_kelas_id_label","<label for=\"".GetInputElementId("jalan_kelas_id", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	...
	$showDetailKeys["pad.pad_tarif_pajak"]["masterkey1"] = $data["id"];		
	$showDetailKeys["pad.pad_tarif_pajak"]["masterkey2"] = $data["id"];		
	$showDetailKeys["pad.pad_terima_line"]["masterkey1"] = $data["id"];		
...
//	rekening_kd_sub - 
	$value = $pageObject->showDBValue("rekening_kd_sub", $data, $keylink);
	$showValues["rekening_kd_sub"] = $value;
	$showFields[] = "rekening_kd_sub";
		$showRawValues["rekening_kd_sub"] = substr($data["rekening_kd_sub"],0,100);

//	rekdenda_id - 
	$value = $pageObject->showDBValue("rekdenda_id", $data, $keylink);
	$showValues["rekdenda_id"] = $value;
	$showFields[] = "rekdenda_id";
		$showRawValues["rekdenda_id"] = substr($data["rekdenda_id"],0,100);

//	masapajak - 
	$value = $pageObject->showDBValue("masapajak", $data, $keylink);
	$showValues["masapajak"] = $value;
	$showFields[] = "masapajak";
		$showRawValues["masapajak"] = substr($data["masapajak"],0,100);

//	jatuhtempo - 
	$value = $pageObject->showDBValue("jatuhtempo", $data, $keylink);
	$showValues["jatuhtempo"] = $value;
	$showFields[] = "jatuhtempo";
		$showRawValues["jatuhtempo"] = substr($data["jatuhtempo"],0,100);
...
//	jalan_kelas_id - 
	$value = $pageObject->showDBValue("jalan_kelas_id", $data, $keylink);
	$showValues["jalan_kelas_id"] = $value;
	$showFields[] = "jalan_kelas_id";
		$showRawValues["jalan_kelas_id"] = substr($data["jalan_kelas_id"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_jenis_pajak";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_jenis_pajak_view.php?".$viewlink."'\"");
