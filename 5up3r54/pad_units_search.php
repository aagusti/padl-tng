...
include("include/pad_units_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_units_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_units_search.htm";
...
$params['shortTableName'] = 'pad_units';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("singkat"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("singkat")] = GetTableURL($pageObject->pSet->getLookupTable("singkat"));
	
	$pageObject->fillFieldToolTips("singkat");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("singkat");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "singkat";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("singkat_label","<label for=\"".GetInputElementId("singkat", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("singkat_label", true);
	
	$xt->assign("singkat_fieldblock", true);
	$xt->assignbyref("singkat_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("singkat_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("singkat_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_singkat", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("singkat");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"singkat", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"singkat", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("urusan_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("urusan_id")] = GetTableURL($pageObject->pSet->getLookupTable("urusan_id"));
	
	$pageObject->fillFieldToolTips("urusan_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("urusan_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "urusan_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("urusan_id_label","<label for=\"".GetInputElementId("urusan_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("urusan_id_label", true);
	
	$xt->assign("urusan_id_fieldblock", true);
	$xt->assignbyref("urusan_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("urusan_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("urusan_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_urusan_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("urusan_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"urusan_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"urusan_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
