...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_tanah_hit_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_air_tanah_hit_inline_edit.htm" : "pad_pad_air_tanah_hit_edit.htm";
...
//	processing vol - begin
	$condition = 1;

	if($condition)
	{
		$control_vol = $pageObject->getControl("vol", $id);
		$control_vol->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing vol - end
//	processing hda - begin
	$condition = 1;

	if($condition)
	{
		$control_hda = $pageObject->getControl("hda", $id);
		$control_hda->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_air_tanah_hit_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_air_tanah_hit_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["vol"] = $evalues["vol"];
	$data["hda"] = $evalues["hda"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("vol_label","<label for=\"".GetInputElementId("vol", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("hda_label","<label for=\"".GetInputElementId("hda", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_air_tanah_hit_view.php?".$viewlink."'\"");
