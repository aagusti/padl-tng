...
include("include/pad_pad_daftar_kd_det_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_daftar_kd_det_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_daftar_kd_det_inline_edit.htm" : "pad_pad_daftar_kd_det_edit.htm";
...
//	processing daftar_id - begin
	$condition = 1;

	if($condition)
	{
		$control_daftar_id = $pageObject->getControl("daftar_id", $id);
		$control_daftar_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing daftar_id - end
//	processing nourut - begin
	$condition = 1;

	if($condition)
	{
		$control_nourut = $pageObject->getControl("nourut", $id);
		$control_nourut->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nourut - end
...
//	processing kamar - begin
	$condition = 1;

	if($condition)
	{
		$control_kamar = $pageObject->getControl("kamar", $id);
		$control_kamar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing volume - end
	...
	//	do event
		...
		{		
			...
			//	processing daftar_id - begin
				...
				if($condition)
				{
					$control_daftar_id->afterSuccessfulSave();
				}
	//	processing daftar_id - end
			//	processing nourut - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_nourut->afterSuccessfulSave();
				}
	//	processing nourut - end
			//	processing notes - begin
							$condition = 1;
	...
	//	processing tarif - end
			//	processing kamar - begin
				...
				if($condition)
				{
					$control_kamar->afterSuccessfulSave();
				}
	//	processing kamar - end
			//	processing volume - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_daftar_kd_det_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_daftar_kd_det_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["daftar_id"] = $evalues["daftar_id"];
	$data["nourut"] = $evalues["nourut"];
	$data["notes"] = $evalues["notes"];
	$data["tarif"] = $evalues["tarif"];
	$data["kamar"] = $evalues["kamar"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("daftar_id"))
		$xt->assign("daftar_id_fieldblock",true);
	else
		$xt->assign("daftar_id_tabfieldblock",true);
	$xt->assign("daftar_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("daftar_id_label","<label for=\"".GetInputElementId("daftar_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("nourut"))
		$xt->assign("nourut_fieldblock",true);
	else
		$xt->assign("nourut_tabfieldblock",true);
	$xt->assign("nourut_label",true);
	if(isEnableSection508())
		$xt->assign_section("nourut_label","<label for=\"".GetInputElementId("nourut", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("kamar"))
		$xt->assign("kamar_fieldblock",true);
	else
		$xt->assign("kamar_tabfieldblock",true);
	$xt->assign("kamar_label",true);
	if(isEnableSection508())
		$xt->assign_section("kamar_label","<label for=\"".GetInputElementId("kamar", $id, PAGE_EDIT)."\">","</label>");
...
//	daftar_id - 
	$value = $pageObject->showDBValue("daftar_id", $data, $keylink);
	$showValues["daftar_id"] = $value;
	$showFields[] = "daftar_id";
		$showRawValues["daftar_id"] = substr($data["daftar_id"],0,100);

//	nourut - 
	$value = $pageObject->showDBValue("nourut", $data, $keylink);
	$showValues["nourut"] = $value;
	$showFields[] = "nourut";
		$showRawValues["nourut"] = substr($data["nourut"],0,100);
...
//	kamar - 
	$value = $pageObject->showDBValue("kamar", $data, $keylink);
	$showValues["kamar"] = $value;
	$showFields[] = "kamar";
		$showRawValues["kamar"] = substr($data["kamar"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_daftar_kd_det";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_daftar_kd_det_view.php?".$viewlink."'\"");
