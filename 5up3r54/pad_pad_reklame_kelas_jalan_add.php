...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_kelas_jalan_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_reklame_kelas_jalan_inline_add.htm";
else
	$templatefile = "pad_pad_reklame_kelas_jalan_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_reklame_kelas_jalan_add.php');
...
//	processing nama - end
//	processing nilai - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_nilai = $pageObject->getControl("nilai", $id);
		$control_nilai->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing kriteria - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_kriteria = $pageObject->getControl("kriteria", $id);
		$control_kriteria->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_kelas_jalan_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_kelas_jalan_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_reklame_kelas_jalan_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["kriteria"]=@$avalues["kriteria"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("nilai_label","<label for=\"".GetInputElementId("nilai", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kriteria_label","<label for=\"".GetInputElementId("kriteria", $id, PAGE_ADD)."\">","</label>");
