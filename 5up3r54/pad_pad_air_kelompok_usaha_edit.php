...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_kelompok_usaha_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_air_kelompok_usaha_inline_edit.htm" : "pad_pad_air_kelompok_usaha_edit.htm";
...
//	processing induk_id - begin
	$condition = 1;

	if($condition)
	{
		$control_induk_id = $pageObject->getControl("induk_id", $id);
		$control_induk_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing induk_id - end
...
//	processing id_old1 - begin
	$condition = 1;

	if($condition)
	{
		$control_id_old1 = $pageObject->getControl("id_old1", $id);
		$control_id_old1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing id_old1 - end
//	processing id_old2 - begin
	$condition = 1;

	if($condition)
	{
		$control_id_old2 = $pageObject->getControl("id_old2", $id);
		$control_id_old2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing id_old2 - end
//	processing id_old3 - begin
	$condition = 1;

	if($condition)
	{
		$control_id_old3 = $pageObject->getControl("id_old3", $id);
		$control_id_old3->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing id_old3 - end
	...
	//	processing level - end
			//	processing induk_id - begin
				...
				if($condition)
				{
					$control_induk_id->afterSuccessfulSave();
				}
	//	processing induk_id - end
			//	processing update_uid - begin
							$condition = 1;
	...
	//	processing created - end
			//	processing id_old1 - begin
				...
				if($condition)
				{
					$control_id_old1->afterSuccessfulSave();
				}
	//	processing id_old1 - end
			//	processing id_old2 - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_id_old2->afterSuccessfulSave();
				}
	//	processing id_old2 - end
			//	processing id_old3 - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_id_old3->afterSuccessfulSave();
				}
	//	processing id_old3 - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_air_kelompok_usaha_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_air_kelompok_usaha_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["induk_id"] = $evalues["induk_id"];
	...
	$data["id_old1"] = $evalues["id_old1"];
	$data["id_old2"] = $evalues["id_old2"];
	$data["id_old3"] = $evalues["id_old3"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("induk_id"))
		$xt->assign("induk_id_fieldblock",true);
	else
		$xt->assign("induk_id_tabfieldblock",true);
	$xt->assign("induk_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("induk_id_label","<label for=\"".GetInputElementId("induk_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("id_old1"))
		$xt->assign("id_old1_fieldblock",true);
	else
		$xt->assign("id_old1_tabfieldblock",true);
	$xt->assign("id_old1_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_old1_label","<label for=\"".GetInputElementId("id_old1", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("id_old2"))
		$xt->assign("id_old2_fieldblock",true);
	else
		$xt->assign("id_old2_tabfieldblock",true);
	$xt->assign("id_old2_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_old2_label","<label for=\"".GetInputElementId("id_old2", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("id_old3"))
		$xt->assign("id_old3_fieldblock",true);
	else
		$xt->assign("id_old3_tabfieldblock",true);
	$xt->assign("id_old3_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_old3_label","<label for=\"".GetInputElementId("id_old3", $id, PAGE_EDIT)."\">","</label>");
...
//	induk_id - 
	$value = $pageObject->showDBValue("induk_id", $data, $keylink);
	$showValues["induk_id"] = $value;
	$showFields[] = "induk_id";
		$showRawValues["induk_id"] = substr($data["induk_id"],0,100);
...
//	id_old1 - 
	$value = $pageObject->showDBValue("id_old1", $data, $keylink);
	$showValues["id_old1"] = $value;
	$showFields[] = "id_old1";
		$showRawValues["id_old1"] = substr($data["id_old1"],0,100);

//	id_old2 - 
	$value = $pageObject->showDBValue("id_old2", $data, $keylink);
	$showValues["id_old2"] = $value;
	$showFields[] = "id_old2";
		$showRawValues["id_old2"] = substr($data["id_old2"],0,100);

//	id_old3 - 
	$value = $pageObject->showDBValue("id_old3", $data, $keylink);
	$showValues["id_old3"] = $value;
	$showFields[] = "id_old3";
		$showRawValues["id_old3"] = substr($data["id_old3"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_air_kelompok_usaha";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_air_kelompok_usaha_view.php?".$viewlink."'\"");
