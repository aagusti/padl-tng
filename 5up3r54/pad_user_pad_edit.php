...
include("include/pad_user_pad_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_user_pad_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_user_pad_inline_edit.htm" : "pad_user_pad_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_user_pad_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_user_pad_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.user_pad";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_user_pad_view.php?".$viewlink."'\"");
