...
include("include/pad_pad_air_tanah_hit_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_air_tanah_hit_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_air_tanah_hit_search.htm";
...
$params['shortTableName'] = 'pad_pad_air_tanah_hit';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("vol"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("vol")] = GetTableURL($pageObject->pSet->getLookupTable("vol"));
	
	$pageObject->fillFieldToolTips("vol");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("vol");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("vol_label","<label for=\"".GetInputElementId("vol", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("vol_label", true);
	
	$xt->assign("vol_fieldblock", true);
	$xt->assignbyref("vol_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("vol_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("vol_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_vol", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("vol");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"vol", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"vol", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("hda"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("hda")] = GetTableURL($pageObject->pSet->getLookupTable("hda"));
	
	$pageObject->fillFieldToolTips("hda");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("hda");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("hda_label","<label for=\"".GetInputElementId("hda", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("hda_label", true);
	
	$xt->assign("hda_fieldblock", true);
	$xt->assignbyref("hda_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("hda_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("hda_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_hda", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("hda");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"hda", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"hda", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
