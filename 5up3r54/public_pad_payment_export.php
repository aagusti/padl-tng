...
$layout->blocks["top"][] = "export";$page_layouts["public_pad_payment_export"] = $layout;
...
$xt->display("public_pad_payment_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=public_pad_payment.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=public_pad_payment.doc");
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=public_pad_payment.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["tagihan"] = $row["tagihan"];
			$values["denda"] = $row["denda"];
			$values["total_bayar"] = $row["total_bayar"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["tagihan"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["denda"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["total_bayar"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["iso_request"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["transmission"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["settlement"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["stan"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ntb"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ntp"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["bank_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["channel_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["bank_ip"]).'"';
			echo $outstr;
		}
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo $values["id"];//echo htmlspecialchars($values["id"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo $values["bank_id"];//echo htmlspecialchars($values["bank_id"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo $values["channel_id"];//echo htmlspecialchars($values["channel_id"]); commented for bug #6823
