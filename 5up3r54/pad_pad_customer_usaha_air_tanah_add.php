...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_customer_usaha_air_tanah_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_customer_usaha_air_tanah_inline_add.htm";
else
	$templatefile = "pad_pad_customer_usaha_air_tanah_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_customer_usaha_air_tanah_add.php');
...
//	processing max_volume - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_max_volume = $pageObject->getControl("max_volume", $id);
		$control_max_volume->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_customer_usaha_air_tanah_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_customer_usaha_air_tanah_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_customer_usaha_air_tanah_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["max_volume"]=@$avalues["max_volume"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("max_volume_label","<label for=\"".GetInputElementId("max_volume", $id, PAGE_ADD)."\">","</label>");
