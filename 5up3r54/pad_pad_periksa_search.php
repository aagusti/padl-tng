...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_periksa_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_periksa_search.htm";
...
$params['shortTableName'] = 'pad_pad_periksa';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("periksa_no"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("periksa_no")] = GetTableURL($pageObject->pSet->getLookupTable("periksa_no"));
	
	$pageObject->fillFieldToolTips("periksa_no");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("periksa_no");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "periksa_no";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("periksa_no_label","<label for=\"".GetInputElementId("periksa_no", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("periksa_no_label", true);
	
	$xt->assign("periksa_no_fieldblock", true);
	$xt->assignbyref("periksa_no_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("periksa_no_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("periksa_no_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_periksa_no", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("periksa_no");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"periksa_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"periksa_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("periksa_tgl"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("periksa_tgl")] = GetTableURL($pageObject->pSet->getLookupTable("periksa_tgl"));
	
	$pageObject->fillFieldToolTips("periksa_tgl");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("periksa_tgl");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "periksa_tgl";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("periksa_tgl_label","<label for=\"".GetInputElementId("periksa_tgl", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("periksa_tgl_label", true);
	
	$xt->assign("periksa_tgl_fieldblock", true);
	$xt->assignbyref("periksa_tgl_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("periksa_tgl_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("periksa_tgl_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_periksa_tgl", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("periksa_tgl");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"periksa_tgl", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"periksa_tgl", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("lhp_no"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("lhp_no")] = GetTableURL($pageObject->pSet->getLookupTable("lhp_no"));
	
	$pageObject->fillFieldToolTips("lhp_no");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("lhp_no");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("lhp_no_label","<label for=\"".GetInputElementId("lhp_no", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("lhp_no_label", true);
	
	$xt->assign("lhp_no_fieldblock", true);
	$xt->assignbyref("lhp_no_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("lhp_no_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("lhp_no_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_lhp_no", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("lhp_no");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"lhp_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"lhp_no", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
