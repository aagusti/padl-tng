...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_reklame_kelas_jalan_export"] = $layout;
...
$xt->display("pad_pad_reklame_kelas_jalan_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_kelas_jalan.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_kelas_jalan.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_kelas_jalan.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["nilai"] = $pageObject->showDBValue("nilai", $row);
			...
			$values["kriteria"] = $pageObject->showDBValue("kriteria", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_kelas_jalan.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nilai"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["kriteria"]).'"';
			if($outstr!="")
				$outstr.=",";
