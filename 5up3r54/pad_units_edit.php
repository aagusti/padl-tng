...
$layout->blocks["top"][] = "details";$page_layouts["pad_units_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_units_inline_edit.htm" : "pad_units_edit.htm";
...
//	processing singkat - begin
	$condition = 1;

	if($condition)
	{
		$control_singkat = $pageObject->getControl("singkat", $id);
		$control_singkat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing singkat - end
//	processing urusan_id - begin
	$condition = 1;

	if($condition)
	{
		$control_urusan_id = $pageObject->getControl("urusan_id", $id);
		$control_urusan_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing urusan_id - end
	...
	//	processing kategori - end
			//	processing singkat - begin
				...
				if($condition)
				{
					$control_singkat->afterSuccessfulSave();
				}
	//	processing singkat - end
			//	processing urusan_id - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_urusan_id->afterSuccessfulSave();
				}
	//	processing urusan_id - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_units_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_units_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["singkat"] = $evalues["singkat"];
	$data["urusan_id"] = $evalues["urusan_id"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("singkat"))
		$xt->assign("singkat_fieldblock",true);
	else
		$xt->assign("singkat_tabfieldblock",true);
	$xt->assign("singkat_label",true);
	if(isEnableSection508())
		$xt->assign_section("singkat_label","<label for=\"".GetInputElementId("singkat", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("urusan_id"))
		$xt->assign("urusan_id_fieldblock",true);
	else
		$xt->assign("urusan_id_tabfieldblock",true);
	$xt->assign("urusan_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("urusan_id_label","<label for=\"".GetInputElementId("urusan_id", $id, PAGE_EDIT)."\">","</label>");
...
//	singkat - 
	$value = $pageObject->showDBValue("singkat", $data, $keylink);
	$showValues["singkat"] = $value;
	$showFields[] = "singkat";
		$showRawValues["singkat"] = substr($data["singkat"],0,100);
...
//	urusan_id - 
	$value = $pageObject->showDBValue("urusan_id", $data, $keylink);
	$showValues["urusan_id"] = $value;
	$showFields[] = "urusan_id";
		$showRawValues["urusan_id"] = substr($data["urusan_id"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_units_view.php?".$viewlink."'\"");
