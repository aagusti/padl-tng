...
$layout->blocks["top"][] = "export";$page_layouts["public_tblpejabat2_export"] = $layout;
...
$xt->display("public_tblpejabat2_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=public_tblpejabat2.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=public_tblpejabat2.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=public_tblpejabat2.xml");
	...
	{
		$values = array();
			$values["id2"] = $pageObject->showDBValue("id2", $row);
			$values["nip2"] = $pageObject->showDBValue("nip2", $row);
			$values["pejabatnm2"] = $pageObject->showDBValue("pejabatnm2", $row);
			$values["jabatan2"] = $pageObject->showDBValue("jabatan2", $row);
			$values["golongan2"] = $pageObject->showDBValue("golongan2", $row);
			$values["pangkat2"] = $pageObject->showDBValue("pangkat2", $row);
			$values["tmt2"] = $pageObject->showDBValue("tmt2", $row);
			$values["enabled2"] = $pageObject->showDBValue("enabled2", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=public_tblpejabat2.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["id2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nip2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["pejabatnm2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["jabatan2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["golongan2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["pangkat2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["tmt2"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["enabled2"]).'"';
			echo $outstr;
		}
