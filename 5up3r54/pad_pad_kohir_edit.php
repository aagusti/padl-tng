...
include("include/pad_pad_kohir_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_kohir_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_kohir_inline_edit.htm" : "pad_pad_kohir_edit.htm";
...
//	processing kohirno - begin
	$condition = 1;

	if($condition)
	{
		$control_kohirno = $pageObject->getControl("kohirno", $id);
		$control_kohirno->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kohirno - end
//	processing kohirtgl - begin
	$condition = 1;

	if($condition)
	{
		$control_kohirtgl = $pageObject->getControl("kohirtgl", $id);
		$control_kohirtgl->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kohirtgl - end
...
//	processing type_id - end
	...
	//	processing usaha_id - end
			//	processing kohirno - begin
				...
				if($condition)
				{
					$control_kohirno->afterSuccessfulSave();
				}
	//	processing kohirno - end
			//	processing kohirtgl - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_kohirtgl->afterSuccessfulSave();
				}
	//	processing kohirtgl - end
			//	processing spt_id - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_kohir_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_kohir_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["kohirno"] = $evalues["kohirno"];
	$data["kohirtgl"] = $evalues["kohirtgl"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("kohirno"))
		$xt->assign("kohirno_fieldblock",true);
	else
		$xt->assign("kohirno_tabfieldblock",true);
	$xt->assign("kohirno_label",true);
	if(isEnableSection508())
		$xt->assign_section("kohirno_label","<label for=\"".GetInputElementId("kohirno", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("kohirtgl"))
		$xt->assign("kohirtgl_fieldblock",true);
	else
		$xt->assign("kohirtgl_tabfieldblock",true);
	$xt->assign("kohirtgl_label",true);
	if(isEnableSection508())
		$xt->assign_section("kohirtgl_label","<label for=\"".GetInputElementId("kohirtgl", $id, PAGE_EDIT)."\">","</label>");
...
//	kohirno - 
	$value = $pageObject->showDBValue("kohirno", $data, $keylink);
	$showValues["kohirno"] = $value;
	$showFields[] = "kohirno";
		$showRawValues["kohirno"] = substr($data["kohirno"],0,100);

//	kohirtgl - Short Date
	$value = $pageObject->showDBValue("kohirtgl", $data, $keylink);
	$showValues["kohirtgl"] = $value;
	$showFields[] = "kohirtgl";
		$showRawValues["kohirtgl"] = substr($data["kohirtgl"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_kohir";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_kohir_view.php?".$viewlink."'\"");
