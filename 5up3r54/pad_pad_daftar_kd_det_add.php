...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_daftar_kd_det_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_daftar_kd_det_inline_add.htm";
else
	$templatefile = "pad_pad_daftar_kd_det_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_daftar_kd_det_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing daftar_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_daftar_id = $pageObject->getControl("daftar_id", $id);
		$control_daftar_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing daftar_id - end
//	processing nourut - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_nourut = $pageObject->getControl("nourut", $id);
		$control_nourut->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing kamar - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_kamar = $pageObject->getControl("kamar", $id);
		$control_kamar->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_daftar_kd_det_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_daftar_kd_det_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_daftar_kd_det_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_daftar")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["daftar_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["daftar_id"]=@$avalues["daftar_id"];
	$defvalues["nourut"]=@$avalues["nourut"];
	$defvalues["notes"]=@$avalues["notes"];
	$defvalues["tarif"]=@$avalues["tarif"];
	$defvalues["kamar"]=@$avalues["kamar"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("daftar_id_label","<label for=\"".GetInputElementId("daftar_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nourut_label","<label for=\"".GetInputElementId("nourut", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kamar_label","<label for=\"".GetInputElementId("kamar", $id, PAGE_ADD)."\">","</label>");
