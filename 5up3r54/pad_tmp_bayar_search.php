...
$layout->blocks["top"][] = "search";$page_layouts["pad_tmp_bayar_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_tmp_bayar_search.htm";
...
$params['shortTableName'] = 'pad_tmp_bayar';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("old_spt_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("old_spt_id")] = GetTableURL($pageObject->pSet->getLookupTable("old_spt_id"));
	
	$pageObject->fillFieldToolTips("old_spt_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("old_spt_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "old_spt_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("old_spt_id_label","<label for=\"".GetInputElementId("old_spt_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("old_spt_id_label", true);
	
	$xt->assign("old_spt_id_fieldblock", true);
	$xt->assignbyref("old_spt_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("old_spt_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("old_spt_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_old_spt_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("old_spt_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"old_spt_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"old_spt_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("wp"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("wp")] = GetTableURL($pageObject->pSet->getLookupTable("wp"));
	
	$pageObject->fillFieldToolTips("wp");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("wp");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("wp_label","<label for=\"".GetInputElementId("wp", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("wp_label", true);
	
	$xt->assign("wp_fieldblock", true);
	$xt->assignbyref("wp_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("wp_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("wp_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_wp", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("wp");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"wp", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"wp", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
