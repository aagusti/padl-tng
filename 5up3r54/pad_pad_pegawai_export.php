...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_pegawai_export"] = $layout;
...
$xt->display("pad_pad_pegawai_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_pegawai.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_pegawai.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_pegawai.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["pangkat"] = $pageObject->showDBValue("pangkat", $row);
			$values["tmt"] = $pageObject->showDBValue("tmt", $row);
			$values["enabled"] = $pageObject->showDBValue("enabled", $row);
			$values["nomor_telp"] = $pageObject->showDBValue("nomor_telp", $row);
			$values["bagian"] = $pageObject->showDBValue("bagian", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_pegawai.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["pangkat"] = $pageObject->getViewControl("pangkat")->showDBValue($row, "");
			$values["tmt"] = $pageObject->getViewControl("tmt")->showDBValue($row, "");
			$values["enabled"] = $pageObject->getViewControl("enabled")->showDBValue($row, "");
			$values["nomor_telp"] = $pageObject->getViewControl("nomor_telp")->showDBValue($row, "");
			$values["bagian"] = $pageObject->getViewControl("bagian")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["pangkat"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nomor_telp"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["bagian"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pangkat").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tmt").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Enabled").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nomor Telp").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Bagian").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["pangkat"]);
					else
						echo $values["pangkat"];
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["nomor_telp"]);
					else
						echo $values["nomor_telp"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["bagian"]);
					else
						echo $values["bagian"];
			echo '</td>';
							echo '<td>';
