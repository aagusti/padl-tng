...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_pegawai_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_pegawai_inline_edit.htm" : "pad_pad_pegawai_edit.htm";
...
//	processing pangkat - begin
	$condition = 1;

	if($condition)
	{
		$control_pangkat = $pageObject->getControl("pangkat", $id);
		$control_pangkat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pangkat - end
...
//	processing nomor_telp - begin
	$condition = 1;

	if($condition)
	{
		$control_nomor_telp = $pageObject->getControl("nomor_telp", $id);
		$control_nomor_telp->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nomor_telp - end
//	processing bagian - begin
	$condition = 1;

	if($condition)
	{
		$control_bagian = $pageObject->getControl("bagian", $id);
		$control_bagian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing bagian - end
...
//	processing update_uid - end
	...
	//	processing golongan - end
			//	processing pangkat - begin
				...
				if($condition)
				{
					$control_pangkat->afterSuccessfulSave();
				}
	//	processing pangkat - end
			//	processing tmt - begin
							$condition = 1;
	...
	//	processing enabled - end
			//	processing nomor_telp - begin
				...
				if($condition)
				{
					$control_nomor_telp->afterSuccessfulSave();
				}
	//	processing nomor_telp - end
			//	processing bagian - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_bagian->afterSuccessfulSave();
				}
	//	processing bagian - end
			//	processing created - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_pegawai_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_pegawai_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["pangkat"] = $evalues["pangkat"];
	$data["tmt"] = $evalues["tmt"];
	$data["enabled"] = $evalues["enabled"];
	$data["nomor_telp"] = $evalues["nomor_telp"];
	$data["bagian"] = $evalues["bagian"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("pangkat"))
		$xt->assign("pangkat_fieldblock",true);
	else
		$xt->assign("pangkat_tabfieldblock",true);
	$xt->assign("pangkat_label",true);
	if(isEnableSection508())
		$xt->assign_section("pangkat_label","<label for=\"".GetInputElementId("pangkat", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("nomor_telp"))
		$xt->assign("nomor_telp_fieldblock",true);
	else
		$xt->assign("nomor_telp_tabfieldblock",true);
	$xt->assign("nomor_telp_label",true);
	if(isEnableSection508())
		$xt->assign_section("nomor_telp_label","<label for=\"".GetInputElementId("nomor_telp", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("bagian"))
		$xt->assign("bagian_fieldblock",true);
	else
		$xt->assign("bagian_tabfieldblock",true);
	$xt->assign("bagian_label",true);
	if(isEnableSection508())
		$xt->assign_section("bagian_label","<label for=\"".GetInputElementId("bagian", $id, PAGE_EDIT)."\">","</label>");
...
//	pangkat - 
	$value = $pageObject->showDBValue("pangkat", $data, $keylink);
	$showValues["pangkat"] = $value;
	$showFields[] = "pangkat";
		$showRawValues["pangkat"] = substr($data["pangkat"],0,100);
...
//	nomor_telp - 
	$value = $pageObject->showDBValue("nomor_telp", $data, $keylink);
	$showValues["nomor_telp"] = $value;
	$showFields[] = "nomor_telp";
		$showRawValues["nomor_telp"] = substr($data["nomor_telp"],0,100);

//	bagian - 
	$value = $pageObject->showDBValue("bagian", $data, $keylink);
	$showValues["bagian"] = $value;
	$showFields[] = "bagian";
		$showRawValues["bagian"] = substr($data["bagian"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_pegawai";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_pegawai_view.php?".$viewlink."'\"");
