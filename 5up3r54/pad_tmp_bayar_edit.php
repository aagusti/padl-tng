...
$layout->blocks["top"][] = "details";$page_layouts["pad_tmp_bayar_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_tmp_bayar_inline_edit.htm" : "pad_tmp_bayar_edit.htm";
...
//	processing old_spt_id - begin
	$condition = 1;

	if($condition)
	{
		$control_old_spt_id = $pageObject->getControl("old_spt_id", $id);
		$control_old_spt_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing old_spt_id - end
//	processing wp - begin
	$condition = 1;

	if($condition)
	{
		$control_wp = $pageObject->getControl("wp", $id);
		$control_wp->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing tgl - end
	...
	//	processing spt_id - end
			//	processing old_spt_id - begin
				...
				if($condition)
				{
					$control_old_spt_id->afterSuccessfulSave();
				}
	//	processing old_spt_id - end
			//	processing wp - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_wp->afterSuccessfulSave();
				}
	//	processing wp - end
			//	processing bunga - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_tmp_bayar_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_tmp_bayar_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["old_spt_id"] = $evalues["old_spt_id"];
	$data["wp"] = $evalues["wp"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("old_spt_id"))
		$xt->assign("old_spt_id_fieldblock",true);
	else
		$xt->assign("old_spt_id_tabfieldblock",true);
	$xt->assign("old_spt_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("old_spt_id_label","<label for=\"".GetInputElementId("old_spt_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("wp"))
		$xt->assign("wp_fieldblock",true);
	else
		$xt->assign("wp_tabfieldblock",true);
	$xt->assign("wp_label",true);
	if(isEnableSection508())
		$xt->assign_section("wp_label","<label for=\"".GetInputElementId("wp", $id, PAGE_EDIT)."\">","</label>");
...
//	old_spt_id - 
	$value = $pageObject->showDBValue("old_spt_id", $data, $keylink);
	$showValues["old_spt_id"] = $value;
	$showFields[] = "old_spt_id";
		$showRawValues["old_spt_id"] = substr($data["old_spt_id"],0,100);

//	wp - 
	$value = $pageObject->showDBValue("wp", $data, $keylink);
	$showValues["wp"] = $value;
	$showFields[] = "wp";
		$showRawValues["wp"] = substr($data["wp"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.tmp_bayar";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_tmp_bayar_view.php?".$viewlink."'\"");
