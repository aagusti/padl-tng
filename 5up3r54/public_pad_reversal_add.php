...
$layout->blocks["top"][] = "details";$page_layouts["public_pad_reversal_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "public_pad_reversal_inline_add.htm";
else
	$templatefile = "public_pad_reversal_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: public_pad_reversal_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'public_pad_reversal_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'public_pad_reversal_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: public_pad_reversal_".$pageObject->getPageType().".php");
