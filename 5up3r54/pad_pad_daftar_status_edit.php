...
include("include/pad_pad_daftar_status_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_daftar_status_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_daftar_status_inline_edit.htm" : "pad_pad_daftar_status_edit.htm";
...
//	processing kode - end
//	processing uraian - begin
	$condition = 1;

	if($condition)
	{
		$control_uraian = $pageObject->getControl("uraian", $id);
		$control_uraian->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing uraian - end
	...
	//	processing kode - end
			//	processing uraian - begin
				...
				if($condition)
				{
					$control_uraian->afterSuccessfulSave();
				}
	//	processing uraian - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_daftar_status_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_daftar_status_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("uraian"))
		$xt->assign("uraian_fieldblock",true);
	else
		$xt->assign("uraian_tabfieldblock",true);
	$xt->assign("uraian_label",true);
	if(isEnableSection508())
		$xt->assign_section("uraian_label","<label for=\"".GetInputElementId("uraian", $id, PAGE_EDIT)."\">","</label>");
...
//	uraian - 
	$value = $pageObject->showDBValue("uraian", $data, $keylink);
	$showValues["uraian"] = $value;
	$showFields[] = "uraian";
		$showRawValues["uraian"] = substr($data["uraian"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_daftar_status";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_daftar_status_view.php?".$viewlink."'\"");
