...
$layout->blocks["top"][] = "details";$page_layouts["app_group_modules_add"] = $layout;
...
if(isset($_REQUEST['afteradd'])){
		header('Location: app_group_modules_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing group_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_group_id = $pageObject->getControl("group_id", $id);
		$control_group_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing group_id - end
//	processing module_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_module_id = $pageObject->getControl("module_id", $id);
		$control_module_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing module_id - end
//	processing reads - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_reads = $pageObject->getControl("reads", $id);
		$control_reads->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing reads - end
//	processing writes - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_writes = $pageObject->getControl("writes", $id);
		$control_writes->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing writes - end
//	processing deletes - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_deletes = $pageObject->getControl("deletes", $id);
		$control_deletes->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing deletes - end
//	processing inserts - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_inserts = $pageObject->getControl("inserts", $id);
		$control_inserts->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'app_group_modules_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'app_group_modules_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: app_group_modules_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="app.groups")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["group_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="app.modules")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["module_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["group_id"]=@$avalues["group_id"];
	$defvalues["module_id"]=@$avalues["module_id"];
	$defvalues["reads"]=@$avalues["reads"];
	$defvalues["writes"]=@$avalues["writes"];
	$defvalues["deletes"]=@$avalues["deletes"];
	$defvalues["inserts"]=@$avalues["inserts"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("group_id_label","<label for=\"".GetInputElementId("group_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("module_id_label","<label for=\"".GetInputElementId("module_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("reads_label","<label for=\"".GetInputElementId("reads", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("writes_label","<label for=\"".GetInputElementId("writes", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("deletes_label","<label for=\"".GetInputElementId("deletes", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("inserts_label","<label for=\"".GetInputElementId("inserts", $id, PAGE_ADD)."\">","</label>");
