...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_rekening_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_rekening_search.htm";
...
$params['shortTableName'] = 'pad_pad_rekening';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("levelid"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("levelid")] = GetTableURL($pageObject->pSet->getLookupTable("levelid"));
	
	$pageObject->fillFieldToolTips("levelid");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("levelid");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "levelid";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("levelid_label","<label for=\"".GetInputElementId("levelid", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("levelid_label", true);
	
	$xt->assign("levelid_fieldblock", true);
	$xt->assignbyref("levelid_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("levelid_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("levelid_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_levelid", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("levelid");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"levelid", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"levelid", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("issummary"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("issummary")] = GetTableURL($pageObject->pSet->getLookupTable("issummary"));
	
	$pageObject->fillFieldToolTips("issummary");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("issummary");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "issummary";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("issummary_label","<label for=\"".GetInputElementId("issummary", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("issummary_label", true);
	
	$xt->assign("issummary_fieldblock", true);
	$xt->assignbyref("issummary_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("issummary_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("issummary_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_issummary", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("issummary");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"issummary", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"issummary", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("defsign"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("defsign")] = GetTableURL($pageObject->pSet->getLookupTable("defsign"));
	
	$pageObject->fillFieldToolTips("defsign");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("defsign");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "defsign";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("defsign_label","<label for=\"".GetInputElementId("defsign", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("defsign_label", true);
	
	$xt->assign("defsign_fieldblock", true);
	$xt->assignbyref("defsign_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("defsign_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("defsign_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_defsign", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("defsign");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"defsign", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"defsign", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("isppkd"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("isppkd")] = GetTableURL($pageObject->pSet->getLookupTable("isppkd"));
	
	$pageObject->fillFieldToolTips("isppkd");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("isppkd");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("isppkd_label","<label for=\"".GetInputElementId("isppkd", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("isppkd_label", true);
	
	$xt->assign("isppkd_fieldblock", true);
	$xt->assignbyref("isppkd_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("isppkd_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("isppkd_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_isppkd", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("isppkd");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"isppkd", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"isppkd", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	if($pageObject->pSet->getLookupTable("insidentil"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("insidentil")] = GetTableURL($pageObject->pSet->getLookupTable("insidentil"));
	
	$pageObject->fillFieldToolTips("insidentil");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("insidentil");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "insidentil";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("insidentil_label","<label for=\"".GetInputElementId("insidentil", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("insidentil_label", true);
	
	$xt->assign("insidentil_fieldblock", true);
	$xt->assignbyref("insidentil_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("insidentil_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("insidentil_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_insidentil", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("insidentil");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"insidentil", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"insidentil", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
