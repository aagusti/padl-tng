...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_nsr_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_reklame_nsr_inline_add.htm";
else
	$templatefile = "pad_pad_reklame_nsr_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_reklame_nsr_add.php');
...
//	processing njopr - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_njopr = $pageObject->getControl("njopr", $id);
		$control_njopr->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing njopr - end
//	processing nspr - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_nspr = $pageObject->getControl("nspr", $id);
		$control_nspr->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nspr - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_nsr_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_nsr_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_reklame_nsr_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["njopr"]=@$avalues["njopr"];
	$defvalues["nspr"]=@$avalues["nspr"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("njopr_label","<label for=\"".GetInputElementId("njopr", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nspr_label","<label for=\"".GetInputElementId("nspr", $id, PAGE_ADD)."\">","</label>");
