...
$layout->blocks["top"][] = "details";$page_layouts["public_ar_payment_detail_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "public_ar_payment_detail_inline_edit.htm" : "public_ar_payment_detail_edit.htm";
...
//	processing ref_kode - begin
	$condition = 1;

	if($condition)
	{
		$control_ref_kode = $pageObject->getControl("ref_kode", $id);
		$control_ref_kode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ref_kode - end
//	processing ref_nama - begin
	$condition = 1;

	if($condition)
	{
		$control_ref_nama = $pageObject->getControl("ref_nama", $id);
		$control_ref_nama->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing ref_nama - end
//	processing tanggal - begin
	$condition = 1;

	if($condition)
	{
		$control_tanggal = $pageObject->getControl("tanggal", $id);
		$control_tanggal->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tanggal - end
//	processing kecamatan_kd - begin
	$condition = 1;

	if($condition)
	{
		$control_kecamatan_kd = $pageObject->getControl("kecamatan_kd", $id);
		$control_kecamatan_kd->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kecamatan_kd - end
//	processing kecamatan_nm - begin
	$condition = 1;

	if($condition)
	{
		$control_kecamatan_nm = $pageObject->getControl("kecamatan_nm", $id);
		$control_kecamatan_nm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kecamatan_nm - end
//	processing kelurahan_kd - begin
	$condition = 1;

	if($condition)
	{
		$control_kelurahan_kd = $pageObject->getControl("kelurahan_kd", $id);
		$control_kelurahan_kd->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kelurahan_kd - end
//	processing kelurahan_nm - begin
	$condition = 1;

	if($condition)
	{
		$control_kelurahan_nm = $pageObject->getControl("kelurahan_nm", $id);
		$control_kelurahan_nm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing kelurahan_nm - end
//	processing is_kota - begin
	$condition = 1;

	if($condition)
	{
		$control_is_kota = $pageObject->getControl("is_kota", $id);
		$control_is_kota->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing is_kota - end
//	processing sumber_data - begin
	$condition = 1;

	if($condition)
	{
		$control_sumber_data = $pageObject->getControl("sumber_data", $id);
		$control_sumber_data->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing sumber_data - end
//	processing sumber_id - begin
	$condition = 1;

	if($condition)
	{
		$control_sumber_id = $pageObject->getControl("sumber_id", $id);
		$control_sumber_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing minggu - begin
	$condition = 1;

	if($condition)
	{
		$control_minggu = $pageObject->getControl("minggu", $id);
		$control_minggu->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing minggu - end
//	processing hari - begin
	$condition = 1;

	if($condition)
	{
		$control_hari = $pageObject->getControl("hari", $id);
		$control_hari->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: public_ar_payment_detail_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["ref_kode"] = $evalues["ref_kode"];
	$data["ref_nama"] = $evalues["ref_nama"];
	$data["tanggal"] = $evalues["tanggal"];
	$data["kecamatan_kd"] = $evalues["kecamatan_kd"];
	$data["kecamatan_nm"] = $evalues["kecamatan_nm"];
	$data["kelurahan_kd"] = $evalues["kelurahan_kd"];
	$data["kelurahan_nm"] = $evalues["kelurahan_nm"];
	$data["is_kota"] = $evalues["is_kota"];
	$data["sumber_data"] = $evalues["sumber_data"];
	$data["sumber_id"] = $evalues["sumber_id"];
	$data["bulan"] = $evalues["bulan"];
	$data["minggu"] = $evalues["minggu"];
	$data["hari"] = $evalues["hari"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("ref_kode_label","<label for=\"".GetInputElementId("ref_kode", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("ref_nama_label","<label for=\"".GetInputElementId("ref_nama", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tanggal_label","<label for=\"".GetInputElementId("tanggal", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kecamatan_kd_label","<label for=\"".GetInputElementId("kecamatan_kd", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kecamatan_nm_label","<label for=\"".GetInputElementId("kecamatan_nm", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kelurahan_kd_label","<label for=\"".GetInputElementId("kelurahan_kd", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kelurahan_nm_label","<label for=\"".GetInputElementId("kelurahan_nm", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("is_kota_label","<label for=\"".GetInputElementId("is_kota", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sumber_data_label","<label for=\"".GetInputElementId("sumber_data", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sumber_id_label","<label for=\"".GetInputElementId("sumber_id", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("minggu_label","<label for=\"".GetInputElementId("minggu", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("hari_label","<label for=\"".GetInputElementId("hari", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='public_ar_payment_detail_view.php?".$viewlink."'\"");
