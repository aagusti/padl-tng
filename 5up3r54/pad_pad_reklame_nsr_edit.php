...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_nsr_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_reklame_nsr_inline_edit.htm" : "pad_pad_reklame_nsr_edit.htm";
...
//	processing njopr - begin
	$condition = 1;

	if($condition)
	{
		$control_njopr = $pageObject->getControl("njopr", $id);
		$control_njopr->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing njopr - end
//	processing nspr - begin
	$condition = 1;

	if($condition)
	{
		$control_nspr = $pageObject->getControl("nspr", $id);
		$control_nspr->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing created - end
	...
	//	processing satuan - end
			//	processing njopr - begin
				...
				if($condition)
				{
					$control_njopr->afterSuccessfulSave();
				}
	//	processing njopr - end
			//	processing nspr - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_nspr->afterSuccessfulSave();
				}
	//	processing nspr - end
			//	processing nsr - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_reklame_nsr_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_reklame_nsr_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["njopr"] = $evalues["njopr"];
	$data["nspr"] = $evalues["nspr"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("njopr"))
		$xt->assign("njopr_fieldblock",true);
	else
		$xt->assign("njopr_tabfieldblock",true);
	$xt->assign("njopr_label",true);
	if(isEnableSection508())
		$xt->assign_section("njopr_label","<label for=\"".GetInputElementId("njopr", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("nspr"))
		$xt->assign("nspr_fieldblock",true);
	else
		$xt->assign("nspr_tabfieldblock",true);
	$xt->assign("nspr_label",true);
	if(isEnableSection508())
		$xt->assign_section("nspr_label","<label for=\"".GetInputElementId("nspr", $id, PAGE_EDIT)."\">","</label>");
...
//	njopr - Number
	$value = $pageObject->showDBValue("njopr", $data, $keylink);
	$showValues["njopr"] = $value;
	$showFields[] = "njopr";
		$showRawValues["njopr"] = substr($data["njopr"],0,100);

//	nspr - Number
	$value = $pageObject->showDBValue("nspr", $data, $keylink);
	$showValues["nspr"] = $value;
	$showFields[] = "nspr";
		$showRawValues["nspr"] = substr($data["nspr"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_reklame_nsr";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_reklame_nsr_view.php?".$viewlink."'\"");
