...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_rekening_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_rekening_inline_add.htm";
else
	$templatefile = "pad_pad_rekening_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_rekening_add.php');
...
//	processing nama - end
//	processing levelid - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_levelid = $pageObject->getControl("levelid", $id);
		$control_levelid->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing levelid - end
//	processing issummary - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_issummary = $pageObject->getControl("issummary", $id);
		$control_issummary->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing issummary - end
//	processing defsign - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_defsign = $pageObject->getControl("defsign", $id);
		$control_defsign->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing defsign - end
//	processing isppkd - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_isppkd = $pageObject->getControl("isppkd", $id);
		$control_isppkd->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing insidentil - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_insidentil = $pageObject->getControl("insidentil", $id);
		$control_insidentil->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_rekening_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_rekening_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_rekening_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["levelid"]=@$avalues["levelid"];
	$defvalues["issummary"]=@$avalues["issummary"];
	$defvalues["defsign"]=@$avalues["defsign"];
	$defvalues["isppkd"]=@$avalues["isppkd"];
	...
	$defvalues["insidentil"]=@$avalues["insidentil"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("levelid_label","<label for=\"".GetInputElementId("levelid", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("issummary_label","<label for=\"".GetInputElementId("issummary", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("defsign_label","<label for=\"".GetInputElementId("defsign", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("isppkd_label","<label for=\"".GetInputElementId("isppkd", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("insidentil_label","<label for=\"".GetInputElementId("insidentil", $id, PAGE_ADD)."\">","</label>");
