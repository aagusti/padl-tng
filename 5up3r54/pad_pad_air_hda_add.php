...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_hda_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_air_hda_inline_add.htm";
else
	$templatefile = "pad_pad_air_hda_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_air_hda_add.php');
...
//	processing hda - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_hda = $pageObject->getControl("hda", $id);
		$control_hda->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing hda - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_air_manfaat")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["manfaat_id"]==""){
			$avalues["manfaat_id"] = prepare_for_db("manfaat_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_air_zona")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["zona_id"]==""){
			$avalues["zona_id"] = prepare_for_db("zona_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing hda - start
			...
			if($inlineAddOption)
			{
				$control_hda->afterSuccessfulSave();
			}
//	processing hda - end
//	processing create_uid - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_air_hda_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_air_hda_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_air_hda_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_air_manfaat")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["manfaat_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_air_zona")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["zona_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("hda"))
		$xt->assign("hda_fieldblock",true);
	else
		$xt->assign("hda_tabfieldblock",true);
	$xt->assign("hda_label",true);
	if(isEnableSection508())
		$xt->assign_section("hda_label","<label for=\"".GetInputElementId("hda", $id, PAGE_ADD)."\">","</label>");
...
//	hda
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("hda", $data, $keylink);
		$showValues["hda"] = $value;
		$showFields[] = "hda";
	}	
//	create_uid
	$display = false;
...
//	id_old3
	...
	{	
		...
		$showRawValues["hda"] = substr($data["hda"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_air_hda";
		$options['firstTime'] = 1;
