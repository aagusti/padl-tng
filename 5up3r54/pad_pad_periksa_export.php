...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_periksa_export"] = $layout;
...
$xt->display("pad_pad_periksa_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_periksa.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_periksa.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_periksa.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["periksa_no"] = $pageObject->showDBValue("periksa_no", $row);
			$values["periksa_tgl"] = $pageObject->showDBValue("periksa_tgl", $row);
			$values["invoice_id"] = $pageObject->showDBValue("invoice_id", $row);
			$values["lhp_no"] = $pageObject->showDBValue("lhp_no", $row);
			$values["keterangan"] = $pageObject->showDBValue("keterangan", $row);
			$values["pokok"] = $pageObject->showDBValue("pokok", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_periksa.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["periksa_no"] = $pageObject->getViewControl("periksa_no")->showDBValue($row, "");
			$values["periksa_tgl"] = $pageObject->getViewControl("periksa_tgl")->showDBValue($row, "");
			$values["invoice_id"] = $pageObject->getViewControl("invoice_id")->showDBValue($row, "");
			$values["lhp_no"] = $pageObject->getViewControl("lhp_no")->showDBValue($row, "");
			$values["keterangan"] = $pageObject->getViewControl("keterangan")->showDBValue($row, "");
			$values["pokok"] = $row["pokok"];
			$values["denda"] = $row["denda"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["periksa_no"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["periksa_tgl"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["invoice_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["lhp_no"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["keterangan"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["pokok"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Periksa No").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Periksa Tgl").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Invoice Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Lhp No").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Keterangan").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pokok").'</td>';	
	...
	else
	{
		echo "<td>"."Id"."</td>";
		echo "<td>"."Tahun"."</td>";
		echo "<td>"."Periksa No"."</td>";
		echo "<td>"."Periksa Tgl"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["lhp_no"] = $pageObject->getViewControl("lhp_no")->showDBValue($row, "");
					$values["keterangan"] = $pageObject->getViewControl("keterangan")->showDBValue($row, "");
					$values["pokok"] = $pageObject->getViewControl("pokok")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									echo $values["periksa_no"];
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["periksa_tgl"]);
					else
						echo $values["periksa_tgl"];
			echo '</td>';
							echo '<td>';
					...
					echo $values["invoice_id"];//echo htmlspecialchars($values["invoice_id"]); commented for bug #6823
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["lhp_no"]);
					else
						echo $values["lhp_no"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
