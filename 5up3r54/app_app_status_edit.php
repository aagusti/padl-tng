...
$layout->blocks["top"][] = "details";$page_layouts["app_app_status_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "app_app_status_inline_edit.htm" : "app_app_status_edit.htm";
...
//	processing app_id - begin
	...
	if($condition)
	{
		$control_app_id = $pageObject->getControl("app_id", $id);
		$control_app_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing app_id - end
//	processing step - begin
	$condition = 1;

	if($condition)
	{
		$control_step = $pageObject->getControl("step", $id);
		$control_step->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing step - end
//	processing operator - begin
	$condition = 1;

	if($condition)
	{
		$control_operator = $pageObject->getControl("operator", $id);
		$control_operator->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing operator - end
//	processing review - begin
	$condition = 1;

	if($condition)
	{
		$control_review = $pageObject->getControl("review", $id);
		$control_review->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing review - end
//	processing manager - begin
	$condition = 1;

	if($condition)
	{
		$control_manager = $pageObject->getControl("manager", $id);
		$control_manager->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing manager - end
	...
	//	processing app_id - end
			//	processing step - begin
				...
				if($condition)
				{
					$control_step->afterSuccessfulSave();
				}
	//	processing step - end
			//	processing operator - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_operator->afterSuccessfulSave();
				}
	//	processing operator - end
			//	processing review - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_review->afterSuccessfulSave();
				}
	//	processing review - end
			//	processing manager - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_manager->afterSuccessfulSave();
				}
	//	processing manager - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: app_app_status_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: app_app_status_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["app_id"] = $evalues["app_id"];
	$data["step"] = $evalues["step"];
	$data["operator"] = $evalues["operator"];
	$data["review"] = $evalues["review"];
	$data["manager"] = $evalues["manager"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("app_id"))
	...
	if(!$pageObject->isAppearOnTabs("step"))
		$xt->assign("step_fieldblock",true);
	else
		$xt->assign("step_tabfieldblock",true);
	$xt->assign("step_label",true);
	if(isEnableSection508())
		$xt->assign_section("step_label","<label for=\"".GetInputElementId("step", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("operator"))
		$xt->assign("operator_fieldblock",true);
	else
		$xt->assign("operator_tabfieldblock",true);
	$xt->assign("operator_label",true);
	if(isEnableSection508())
		$xt->assign_section("operator_label","<label for=\"".GetInputElementId("operator", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("review"))
		$xt->assign("review_fieldblock",true);
	else
		$xt->assign("review_tabfieldblock",true);
	$xt->assign("review_label",true);
	if(isEnableSection508())
		$xt->assign_section("review_label","<label for=\"".GetInputElementId("review", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("manager_label","<label for=\"".GetInputElementId("manager", $id, PAGE_EDIT)."\">","</label>");
...
//	app_id - 
	$value = $pageObject->showDBValue("app_id", $data, $keylink);
	$showValues["app_id"] = $value;
	$showFields[] = "app_id";
		$showRawValues["app_id"] = substr($data["app_id"],0,100);

//	step - 
	$value = $pageObject->showDBValue("step", $data, $keylink);
	$showValues["step"] = $value;
	$showFields[] = "step";
		$showRawValues["step"] = substr($data["step"],0,100);

//	operator - 
	$value = $pageObject->showDBValue("operator", $data, $keylink);
	$showValues["operator"] = $value;
	$showFields[] = "operator";
		$showRawValues["operator"] = substr($data["operator"],0,100);

//	review - 
	$value = $pageObject->showDBValue("review", $data, $keylink);
	$showValues["review"] = $value;
	$showFields[] = "review";
		$showRawValues["review"] = substr($data["review"],0,100);

//	manager - 
	$value = $pageObject->showDBValue("manager", $data, $keylink);
	$showValues["manager"] = $value;
	$showFields[] = "manager";
		$showRawValues["manager"] = substr($data["manager"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "app.app_status";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='app_app_status_view.php?".$viewlink."'\"");
