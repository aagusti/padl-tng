...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_report_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_report_inline_add.htm";
else
	$templatefile = "pad_pad_report_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_report_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing spt_type_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_spt_type_id = $pageObject->getControl("spt_type_id", $id);
		$control_spt_type_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing sptnm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sptnm = $pageObject->getControl("sptnm", $id);
		$control_sptnm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sptnm - end
//	processing sknm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sknm = $pageObject->getControl("sknm", $id);
		$control_sknm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sknm - end
//	processing nhnm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_nhnm = $pageObject->getControl("nhnm", $id);
		$control_nhnm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nhnm - end
//	processing sspdnm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sspdnm = $pageObject->getControl("sspdnm", $id);
		$control_sspdnm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sspdnm - end
//	processing stpnm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_stpnm = $pageObject->getControl("stpnm", $id);
		$control_stpnm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing stpnm - end
//	processing kartudtnm - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_kartudtnm = $pageObject->getControl("kartudtnm", $id);
		$control_kartudtnm->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_report_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_report_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_report_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["spt_type_id"]=@$avalues["spt_type_id"];
	$defvalues["usaha_id"]=@$avalues["usaha_id"];
	$defvalues["sptnm"]=@$avalues["sptnm"];
	$defvalues["sknm"]=@$avalues["sknm"];
	$defvalues["nhnm"]=@$avalues["nhnm"];
	$defvalues["sspdnm"]=@$avalues["sspdnm"];
	$defvalues["stpnm"]=@$avalues["stpnm"];
	$defvalues["kartudtnm"]=@$avalues["kartudtnm"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("spt_type_id_label","<label for=\"".GetInputElementId("spt_type_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sptnm_label","<label for=\"".GetInputElementId("sptnm", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sknm_label","<label for=\"".GetInputElementId("sknm", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nhnm_label","<label for=\"".GetInputElementId("nhnm", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sspdnm_label","<label for=\"".GetInputElementId("sspdnm", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("stpnm_label","<label for=\"".GetInputElementId("stpnm", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("kartudtnm_label","<label for=\"".GetInputElementId("kartudtnm", $id, PAGE_ADD)."\">","</label>");
