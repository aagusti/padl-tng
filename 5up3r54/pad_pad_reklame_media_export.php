...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_reklame_media_export"] = $layout;
...
$xt->display("pad_pad_reklame_media_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_media.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_media.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_media.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["jenis_pajak_id"] = $pageObject->showDBValue("jenis_pajak_id", $row);
			$values["media"] = $pageObject->showDBValue("media", $row);
			$values["singkatan"] = $pageObject->showDBValue("singkatan", $row);
			$values["masa"] = $pageObject->showDBValue("masa", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_media.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["jenis_pajak_id"] = $pageObject->getViewControl("jenis_pajak_id")->showDBValue($row, "");
			$values["media"] = $pageObject->getViewControl("media")->showDBValue($row, "");
			$values["singkatan"] = $pageObject->getViewControl("singkatan")->showDBValue($row, "");
			$values["masa"] = $pageObject->getViewControl("masa")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["jenis_pajak_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["media"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["singkatan"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["masa"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Jenis Pajak Id").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Media").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Singkatan").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Masa").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["media"]);
					else
						echo $values["media"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["masa"]);
					else
						echo $values["masa"];
			echo '</td>';
							if($_REQUEST["type"]=="excel")
