...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_tanah_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_air_tanah_inline_edit.htm" : "pad_pad_air_tanah_edit.htm";
...
//	processing awal - begin
	$condition = 1;

	if($condition)
	{
		$control_awal = $pageObject->getControl("awal", $id);
		$control_awal->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing awal - end
//	processing akhir - begin
	$condition = 1;

	if($condition)
	{
		$control_akhir = $pageObject->getControl("akhir", $id);
		$control_akhir->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_air_tanah_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_air_tanah_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["awal"] = $evalues["awal"];
	$data["akhir"] = $evalues["akhir"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("awal_label","<label for=\"".GetInputElementId("awal", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("akhir_label","<label for=\"".GetInputElementId("akhir", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_air_tanah_view.php?".$viewlink."'\"");
