...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_stpd_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_stpd_inline_edit.htm" : "pad_pad_stpd_edit.htm";
...
//	processing stpdno - begin
	$condition = 1;

	if($condition)
	{
		$control_stpdno = $pageObject->getControl("stpdno", $id);
		$control_stpdno->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing stpdno - end
//	processing stpdtgl - begin
	$condition = 1;

	if($condition)
	{
		$control_stpdtgl = $pageObject->getControl("stpdtgl", $id);
		$control_stpdtgl->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing stpdcount - begin
	$condition = 1;

	if($condition)
	{
		$control_stpdcount = $pageObject->getControl("stpdcount", $id);
		$control_stpdcount->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_stpd_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_stpd_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("stpdno_label","<label for=\"".GetInputElementId("stpdno", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("stpdtgl_label","<label for=\"".GetInputElementId("stpdtgl", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("stpdcount_label","<label for=\"".GetInputElementId("stpdcount", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_stpd_view.php?".$viewlink."'\"");
