...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_reklame_nsr_export"] = $layout;
...
$xt->display("pad_pad_reklame_nsr_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_nsr.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_nsr.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_nsr.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["njopr"] = $pageObject->showDBValue("njopr", $row);
			$values["nspr"] = $pageObject->showDBValue("nspr", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_nsr.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["njopr"] = $row["njopr"];
			$values["nspr"] = $row["nspr"];
			$values["nsr"] = $row["nsr"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["kelas_jalan_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["media_id"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["satuan"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["njopr"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nspr"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["nsr"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Njopr").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nspr").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["media_id"] = $pageObject->getViewControl("media_id")->showDBValue($row, "");
					$values["satuan"] = $pageObject->getViewControl("satuan")->showDBValue($row, "");
					$values["njopr"] = $pageObject->getViewControl("njopr")->showDBValue($row, "");
					$values["nspr"] = $pageObject->getViewControl("nspr")->showDBValue($row, "");
