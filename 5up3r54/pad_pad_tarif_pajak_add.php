...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_tarif_pajak_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_tarif_pajak_inline_add.htm";
else
	$templatefile = "pad_pad_tarif_pajak_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_tarif_pajak_add.php');
...
//	processing reklame - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_reklame = $pageObject->getControl("reklame", $id);
		$control_reklame->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_tarif_pajak_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_tarif_pajak_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_tarif_pajak_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="pad.pad_jenis_pajak")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["pajak_id"] = @$_SESSION[$sessionPrefix."_masterkey2"];	
...
if($readavalues)
{
	...
	$defvalues["reklame"]=@$avalues["reklame"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("reklame_label","<label for=\"".GetInputElementId("reklame", $id, PAGE_ADD)."\">","</label>");
