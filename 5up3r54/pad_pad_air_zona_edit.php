...
include("include/pad_pad_air_zona_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_zona_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_air_zona_inline_edit.htm" : "pad_pad_air_zona_edit.htm";
...
//	processing nama - end
//	processing indeks - begin
	$condition = 1;

	if($condition)
	{
		$control_indeks = $pageObject->getControl("indeks", $id);
		$control_indeks->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing indeks - end
...
//	processing created - end
	...
	//	processing nama - end
			//	processing indeks - begin
				...
				if($condition)
				{
					$control_indeks->afterSuccessfulSave();
				}
	//	processing indeks - end
			//	processing update_uid - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_air_zona_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_air_zona_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("indeks"))
		$xt->assign("indeks_fieldblock",true);
	else
		$xt->assign("indeks_tabfieldblock",true);
	$xt->assign("indeks_label",true);
	if(isEnableSection508())
		$xt->assign_section("indeks_label","<label for=\"".GetInputElementId("indeks", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	$showDetailKeys["pad.pad_air_hda"]["masterkey1"] = $data["id"];		
...
//	indeks - Number
	$value = $pageObject->showDBValue("indeks", $data, $keylink);
	$showValues["indeks"] = $value;
	$showFields[] = "indeks";
		$showRawValues["indeks"] = substr($data["indeks"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_air_zona";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_air_zona_view.php?".$viewlink."'\"");
