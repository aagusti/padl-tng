...
include("include/pad_pad_terima_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_terima_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_terima_inline_edit.htm" : "pad_pad_terima_edit.htm";
...
//	processing terimano - begin
	$condition = 1;

	if($condition)
	{
		$control_terimano = $pageObject->getControl("terimano", $id);
		$control_terimano->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing terimano - end
...
//	processing jmlterima - begin
	$condition = 1;

	if($condition)
	{
		$control_jmlterima = $pageObject->getControl("jmlterima", $id);
		$control_jmlterima->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing jmlterima - end
...
//	processing nobukti - begin
	$condition = 1;

	if($condition)
	{
		$control_nobukti = $pageObject->getControl("nobukti", $id);
		$control_nobukti->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing nobukti - end
...
//	processing write_uid - end
	...
	//	processing tahun - end
			//	processing terimano - begin
				...
				if($condition)
				{
					$control_terimano->afterSuccessfulSave();
				}
	//	processing terimano - end
			//	processing terimatgl - begin
							$condition = 1;
	...
	//	processing terimatgl - end
			//	processing jmlterima - begin
				...
				if($condition)
				{
					$control_jmlterima->afterSuccessfulSave();
				}
	//	processing jmlterima - end
			//	processing customer_id - begin
							$condition = 1;
	...
	//	processing npwpd - end
			//	processing nobukti - begin
				...
				if($condition)
				{
					$control_nobukti->afterSuccessfulSave();
				}
	//	processing nobukti - end
			//	processing notes - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_terima_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_terima_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("terimano"))
		$xt->assign("terimano_fieldblock",true);
	else
		$xt->assign("terimano_tabfieldblock",true);
	$xt->assign("terimano_label",true);
	if(isEnableSection508())
		$xt->assign_section("terimano_label","<label for=\"".GetInputElementId("terimano", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("jmlterima"))
		$xt->assign("jmlterima_fieldblock",true);
	else
		$xt->assign("jmlterima_tabfieldblock",true);
	$xt->assign("jmlterima_label",true);
	if(isEnableSection508())
		$xt->assign_section("jmlterima_label","<label for=\"".GetInputElementId("jmlterima", $id, PAGE_EDIT)."\">","</label>");
	...
	if(!$pageObject->isAppearOnTabs("nobukti"))
		$xt->assign("nobukti_fieldblock",true);
	else
		$xt->assign("nobukti_tabfieldblock",true);
	$xt->assign("nobukti_label",true);
	if(isEnableSection508())
		$xt->assign_section("nobukti_label","<label for=\"".GetInputElementId("nobukti", $id, PAGE_EDIT)."\">","</label>");
...
//	terimano - 
	$value = $pageObject->showDBValue("terimano", $data, $keylink);
	$showValues["terimano"] = $value;
	$showFields[] = "terimano";
		$showRawValues["terimano"] = substr($data["terimano"],0,100);
...
//	jmlterima - Number
	$value = $pageObject->showDBValue("jmlterima", $data, $keylink);
	$showValues["jmlterima"] = $value;
	$showFields[] = "jmlterima";
		$showRawValues["jmlterima"] = substr($data["jmlterima"],0,100);
...
//	nobukti - 
	$value = $pageObject->showDBValue("nobukti", $data, $keylink);
	$showValues["nobukti"] = $value;
	$showFields[] = "nobukti";
		$showRawValues["nobukti"] = substr($data["nobukti"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_terima";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_terima_view.php?".$viewlink."'\"");
