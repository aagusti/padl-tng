...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_teguran_add"] = $layout;
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_teguran_add.php');
...
//	processing teguran_no - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_teguran_no = $pageObject->getControl("teguran_no", $id);
		$control_teguran_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing cetak_ke - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_cetak_ke = $pageObject->getControl("cetak_ke", $id);
		$control_cetak_ke->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_teguran_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_teguran_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_teguran_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("teguran_no_label","<label for=\"".GetInputElementId("teguran_no", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cetak_ke_label","<label for=\"".GetInputElementId("cetak_ke", $id, PAGE_ADD)."\">","</label>");
