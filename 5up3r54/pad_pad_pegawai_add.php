...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_pegawai_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_pegawai_inline_add.htm";
else
	$templatefile = "pad_pad_pegawai_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_pegawai_add.php');
...
//	processing pangkat - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pangkat = $pageObject->getControl("pangkat", $id);
		$control_pangkat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing nomor_telp - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_nomor_telp = $pageObject->getControl("nomor_telp", $id);
		$control_nomor_telp->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing nomor_telp - end
//	processing bagian - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_bagian = $pageObject->getControl("bagian", $id);
		$control_bagian->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_pegawai_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_pegawai_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_pegawai_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["pangkat"]=@$avalues["pangkat"];
	$defvalues["tmt"]=@$avalues["tmt"];
	$defvalues["enabled"]=@$avalues["enabled"];
	$defvalues["nomor_telp"]=@$avalues["nomor_telp"];
	$defvalues["bagian"]=@$avalues["bagian"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("pangkat_label","<label for=\"".GetInputElementId("pangkat", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nomor_telp_label","<label for=\"".GetInputElementId("nomor_telp", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("bagian_label","<label for=\"".GetInputElementId("bagian", $id, PAGE_ADD)."\">","</label>");
