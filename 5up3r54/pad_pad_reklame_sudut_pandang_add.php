...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_sudut_pandang_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_reklame_sudut_pandang_inline_add.htm";
else
	$templatefile = "pad_pad_reklame_sudut_pandang_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_reklame_sudut_pandang_add.php');
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_sudut_pandang_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_sudut_pandang_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_reklame_sudut_pandang_".$pageObject->getPageType().".php");
