...
$layout->blocks["top"][] = "details";$page_layouts["public_tmp_bank2_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "public_tmp_bank2_inline_edit.htm" : "public_tmp_bank2_edit.htm";
...
//	processing bayar - begin
	$condition = 1;

	if($condition)
	{
		$control_bayar = $pageObject->getControl("bayar", $id);
		$control_bayar->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing bayar - end
//	processing periode_1 - begin
	$condition = 1;

	if($condition)
	{
		$control_periode_1 = $pageObject->getControl("periode_1", $id);
		$control_periode_1->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing periode_1 - end
//	processing periode_2 - begin
	$condition = 1;

	if($condition)
	{
		$control_periode_2 = $pageObject->getControl("periode_2", $id);
		$control_periode_2->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing spt_id_old - begin
	$condition = 1;

	if($condition)
	{
		$control_spt_id_old = $pageObject->getControl("spt_id_old", $id);
		$control_spt_id_old->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing spt_id_old - end
//	processing judul - begin
	$condition = 1;

	if($condition)
	{
		$control_judul = $pageObject->getControl("judul", $id);
		$control_judul->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing judul - end
//	processing lokasi - begin
	$condition = 1;

	if($condition)
	{
		$control_lokasi = $pageObject->getControl("lokasi", $id);
		$control_lokasi->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing j - begin
	...
	if($condition)
	{
		...
		$control_j->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing p - begin
	...
	if($condition)
	{
		...
		$control_p->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing l - begin
	...
	if($condition)
	{
		...
		$control_l->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing t - begin
	...
	if($condition)
	{
		...
		$control_t->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing t - end
//	processing sspd_id - begin
	$condition = 1;

	if($condition)
	{
		$control_sspd_id = $pageObject->getControl("sspd_id", $id);
		$control_sspd_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: public_tmp_bank2_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: public_tmp_bank2_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["bayar"] = $evalues["bayar"];
	$data["periode_1"] = $evalues["periode_1"];
	$data["periode_2"] = $evalues["periode_2"];
	$data["keterangan"] = $evalues["keterangan"];
	$data["spt_id_old"] = $evalues["spt_id_old"];
	$data["judul"] = $evalues["judul"];
	$data["lokasi"] = $evalues["lokasi"];
	$data["j"] = $evalues["j"];
	$data["p"] = $evalues["p"];
	$data["l"] = $evalues["l"];
	$data["t"] = $evalues["t"];
	$data["sspd_id"] = $evalues["sspd_id"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("bayar_label","<label for=\"".GetInputElementId("bayar", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("periode_1_label","<label for=\"".GetInputElementId("periode_1", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("periode_2_label","<label for=\"".GetInputElementId("periode_2", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("spt_id_old_label","<label for=\"".GetInputElementId("spt_id_old", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("judul_label","<label for=\"".GetInputElementId("judul", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("lokasi_label","<label for=\"".GetInputElementId("lokasi", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("j_label","<label for=\"".GetInputElementId("j", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("p_label","<label for=\"".GetInputElementId("p", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("l_label","<label for=\"".GetInputElementId("l", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("t_label","<label for=\"".GetInputElementId("t", $id, PAGE_EDIT)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sspd_id_label","<label for=\"".GetInputElementId("sspd_id", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='public_tmp_bank2_view.php?".$viewlink."'\"");
