...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_rekening_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_rekening_inline_edit.htm" : "pad_pad_rekening_edit.htm";
...
//	processing nama - end
//	processing levelid - begin
	$condition = 1;

	if($condition)
	{
		$control_levelid = $pageObject->getControl("levelid", $id);
		$control_levelid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing levelid - end
//	processing issummary - begin
	$condition = 1;

	if($condition)
	{
		$control_issummary = $pageObject->getControl("issummary", $id);
		$control_issummary->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing issummary - end
//	processing defsign - begin
	$condition = 1;

	if($condition)
	{
		$control_defsign = $pageObject->getControl("defsign", $id);
		$control_defsign->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing defsign - end
//	processing isppkd - begin
	$condition = 1;

	if($condition)
	{
		$control_isppkd = $pageObject->getControl("isppkd", $id);
		$control_isppkd->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing isppkd - end
...
//	processing insidentil - begin
	$condition = 1;

	if($condition)
	{
		$control_insidentil = $pageObject->getControl("insidentil", $id);
		$control_insidentil->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing insidentil - end
	...
	//	processing nama - end
			//	processing levelid - begin
				...
				if($condition)
				{
					$control_levelid->afterSuccessfulSave();
				}
	//	processing levelid - end
			//	processing issummary - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_issummary->afterSuccessfulSave();
				}
	//	processing issummary - end
			//	processing defsign - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_defsign->afterSuccessfulSave();
				}
	//	processing defsign - end
			//	processing isppkd - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_isppkd->afterSuccessfulSave();
				}
	//	processing isppkd - end
			//	processing tmt - begin
							$condition = 1;
	...
	//	processing update_uid - end
			//	processing insidentil - begin
				...
				if($condition)
				{
					$control_insidentil->afterSuccessfulSave();
				}
	//	processing insidentil - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_rekening_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["levelid"] = $evalues["levelid"];
	$data["issummary"] = $evalues["issummary"];
	$data["defsign"] = $evalues["defsign"];
	$data["isppkd"] = $evalues["isppkd"];
	...
	$data["insidentil"] = $evalues["insidentil"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("levelid_label",true);
	if(isEnableSection508())
		$xt->assign_section("levelid_label","<label for=\"".GetInputElementId("levelid", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("issummary"))
		$xt->assign("issummary_fieldblock",true);
	else
		$xt->assign("issummary_tabfieldblock",true);
	$xt->assign("issummary_label",true);
	if(isEnableSection508())
		$xt->assign_section("issummary_label","<label for=\"".GetInputElementId("issummary", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("defsign"))
		$xt->assign("defsign_fieldblock",true);
	else
		$xt->assign("defsign_tabfieldblock",true);
	$xt->assign("defsign_label",true);
	if(isEnableSection508())
		$xt->assign_section("defsign_label","<label for=\"".GetInputElementId("defsign", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("isppkd"))
		$xt->assign("isppkd_fieldblock",true);
	else
		$xt->assign("isppkd_tabfieldblock",true);
	$xt->assign("isppkd_label",true);
	if(isEnableSection508())
		$xt->assign_section("isppkd_label","<label for=\"".GetInputElementId("isppkd", $id, PAGE_EDIT)."\">","</label>");
	...
	$xt->assign("insidentil_label",true);
	if(isEnableSection508())
		$xt->assign_section("insidentil_label","<label for=\"".GetInputElementId("insidentil", $id, PAGE_EDIT)."\">","</label>");
...
//	detail tables
	...
	$showDetailKeys["pad.pad_anggaran"]["masterkey1"] = $data["id"];		
	$showDetailKeys["pad.pad_jenis_pajak"]["masterkey1"] = $data["id"];		
...
//	levelid - 
	...
	$showValues["levelid"] = $value;
	$showFields[] = "levelid";
		$showRawValues["levelid"] = substr($data["levelid"],0,100);

//	issummary - 
	$value = $pageObject->showDBValue("issummary", $data, $keylink);
	$showValues["issummary"] = $value;
	$showFields[] = "issummary";
		$showRawValues["issummary"] = substr($data["issummary"],0,100);

//	defsign - 
	$value = $pageObject->showDBValue("defsign", $data, $keylink);
	$showValues["defsign"] = $value;
	$showFields[] = "defsign";
		$showRawValues["defsign"] = substr($data["defsign"],0,100);

//	isppkd - 
	$value = $pageObject->showDBValue("isppkd", $data, $keylink);
	$showValues["isppkd"] = $value;
	$showFields[] = "isppkd";
		$showRawValues["isppkd"] = substr($data["isppkd"],0,100);
...
//	insidentil - 
	...
	$showValues["insidentil"] = $value;
	$showFields[] = "insidentil";
		$showRawValues["insidentil"] = substr($data["insidentil"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_rekening_view.php?".$viewlink."'\"");
