...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_daftar_status_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_daftar_status_inline_add.htm";
else
	$templatefile = "pad_pad_daftar_status_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_daftar_status_add.php');
...
//	processing kode - end
//	processing uraian - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_uraian = $pageObject->getControl("uraian", $id);
		$control_uraian->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_daftar_status_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_daftar_status_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_daftar_status_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["uraian"]=@$avalues["uraian"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("uraian_label","<label for=\"".GetInputElementId("uraian", $id, PAGE_ADD)."\">","</label>");
