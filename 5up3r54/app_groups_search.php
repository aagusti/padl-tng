...
$layout->blocks["top"][] = "search";$page_layouts["app_groups_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "app_groups_search.htm";
...
$params['shortTableName'] = 'app_groups';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("locked"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("locked")] = GetTableURL($pageObject->pSet->getLookupTable("locked"));
	
	$pageObject->fillFieldToolTips("locked");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("locked");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("locked_label","<label for=\"".GetInputElementId("locked", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("locked_label", true);
	
	$xt->assign("locked_fieldblock", true);
	$xt->assignbyref("locked_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("locked_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("locked_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_locked", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("locked");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"locked", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"locked", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
