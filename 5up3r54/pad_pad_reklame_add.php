...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_reklame_inline_add.htm";
else
	$templatefile = "pad_pad_reklame_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_reklame_add.php');
...
//	processing pemilik_nama - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pemilik_nama = $pageObject->getControl("pemilik_nama", $id);
		$control_pemilik_nama->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pemilik_nama - end
//	processing pemilik_alamat - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pemilik_alamat = $pageObject->getControl("pemilik_alamat", $id);
		$control_pemilik_alamat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pemilik_alamat - end
//	processing pemilik_kecamatan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pemilik_kecamatan = $pageObject->getControl("pemilik_kecamatan", $id);
		$control_pemilik_kecamatan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pemilik_kecamatan - end
//	processing pemilik_kelurahan - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pemilik_kelurahan = $pageObject->getControl("pemilik_kelurahan", $id);
		$control_pemilik_kelurahan->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing pemilik_kelurahan - end
//	processing pemilik_kota - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_pemilik_kota = $pageObject->getControl("pemilik_kota", $id);
		$control_pemilik_kota->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_reklame_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_reklame_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["pemilik_nama"]=@$avalues["pemilik_nama"];
	$defvalues["pemilik_alamat"]=@$avalues["pemilik_alamat"];
	$defvalues["pemilik_kecamatan"]=@$avalues["pemilik_kecamatan"];
	$defvalues["pemilik_kelurahan"]=@$avalues["pemilik_kelurahan"];
	$defvalues["pemilik_kota"]=@$avalues["pemilik_kota"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("pemilik_nama_label","<label for=\"".GetInputElementId("pemilik_nama", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pemilik_alamat_label","<label for=\"".GetInputElementId("pemilik_alamat", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pemilik_kecamatan_label","<label for=\"".GetInputElementId("pemilik_kecamatan", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pemilik_kelurahan_label","<label for=\"".GetInputElementId("pemilik_kelurahan", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("pemilik_kota_label","<label for=\"".GetInputElementId("pemilik_kota", $id, PAGE_ADD)."\">","</label>");
