...
include("include/pad_pad_daftar_hist_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["pad_pad_daftar_hist_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "pad_pad_daftar_hist_search.htm";
...
$params['shortTableName'] = 'pad_pad_daftar_hist';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("daftar_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("daftar_id")] = GetTableURL($pageObject->pSet->getLookupTable("daftar_id"));
	
	$pageObject->fillFieldToolTips("daftar_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("daftar_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "daftar_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("daftar_id_label","<label for=\"".GetInputElementId("daftar_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("daftar_id_label", true);
	
	$xt->assign("daftar_id_fieldblock", true);
	$xt->assignbyref("daftar_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("daftar_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("daftar_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_daftar_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("daftar_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"daftar_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"daftar_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("status_id"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("status_id")] = GetTableURL($pageObject->pSet->getLookupTable("status_id"));
	
	$pageObject->fillFieldToolTips("status_id");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("status_id");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "status_id";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("status_id_label","<label for=\"".GetInputElementId("status_id", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("status_id_label", true);
	
	$xt->assign("status_id_fieldblock", true);
	$xt->assignbyref("status_id_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("status_id_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("status_id_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_status_id", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("status_id");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"status_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"status_id", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
