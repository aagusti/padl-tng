...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_reklame_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_reklame_inline_edit.htm" : "pad_pad_reklame_edit.htm";
...
//	processing kode - begin
	...
	if($condition)
	{
		...
		$control_kode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing pemilik_nama - begin
	$condition = 1;

	if($condition)
	{
		$control_pemilik_nama = $pageObject->getControl("pemilik_nama", $id);
		$control_pemilik_nama->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pemilik_nama - end
//	processing pemilik_alamat - begin
	$condition = 1;

	if($condition)
	{
		$control_pemilik_alamat = $pageObject->getControl("pemilik_alamat", $id);
		$control_pemilik_alamat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pemilik_alamat - end
//	processing pemilik_kecamatan - begin
	$condition = 1;

	if($condition)
	{
		$control_pemilik_kecamatan = $pageObject->getControl("pemilik_kecamatan", $id);
		$control_pemilik_kecamatan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pemilik_kecamatan - end
//	processing pemilik_kelurahan - begin
	$condition = 1;

	if($condition)
	{
		$control_pemilik_kelurahan = $pageObject->getControl("pemilik_kelurahan", $id);
		$control_pemilik_kelurahan->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pemilik_kelurahan - end
//	processing pemilik_kota - begin
	$condition = 1;

	if($condition)
	{
		$control_pemilik_kota = $pageObject->getControl("pemilik_kota", $id);
		$control_pemilik_kota->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing pemilik_kota - end
...
//	processing create_uid - end
	...
	//	processing longitude - end
			//	processing pemilik_nama - begin
				...
				if($condition)
				{
					$control_pemilik_nama->afterSuccessfulSave();
				}
	//	processing pemilik_nama - end
			//	processing pemilik_alamat - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_pemilik_alamat->afterSuccessfulSave();
				}
	//	processing pemilik_alamat - end
			//	processing pemilik_kecamatan - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_pemilik_kecamatan->afterSuccessfulSave();
				}
	//	processing pemilik_kecamatan - end
			//	processing pemilik_kelurahan - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_pemilik_kelurahan->afterSuccessfulSave();
				}
	//	processing pemilik_kelurahan - end
			//	processing pemilik_kota - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_pemilik_kota->afterSuccessfulSave();
				}
	//	processing pemilik_kota - end
			//	processing updated - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_reklame_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_reklame_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["pemilik_nama"] = $evalues["pemilik_nama"];
	$data["pemilik_alamat"] = $evalues["pemilik_alamat"];
	$data["pemilik_kecamatan"] = $evalues["pemilik_kecamatan"];
	$data["pemilik_kelurahan"] = $evalues["pemilik_kelurahan"];
	$data["pemilik_kota"] = $evalues["pemilik_kota"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("pemilik_nama"))
		$xt->assign("pemilik_nama_fieldblock",true);
	else
		$xt->assign("pemilik_nama_tabfieldblock",true);
	$xt->assign("pemilik_nama_label",true);
	if(isEnableSection508())
		$xt->assign_section("pemilik_nama_label","<label for=\"".GetInputElementId("pemilik_nama", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("pemilik_alamat"))
		$xt->assign("pemilik_alamat_fieldblock",true);
	else
		$xt->assign("pemilik_alamat_tabfieldblock",true);
	$xt->assign("pemilik_alamat_label",true);
	if(isEnableSection508())
		$xt->assign_section("pemilik_alamat_label","<label for=\"".GetInputElementId("pemilik_alamat", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("pemilik_kecamatan"))
		$xt->assign("pemilik_kecamatan_fieldblock",true);
	else
		$xt->assign("pemilik_kecamatan_tabfieldblock",true);
	$xt->assign("pemilik_kecamatan_label",true);
	if(isEnableSection508())
		$xt->assign_section("pemilik_kecamatan_label","<label for=\"".GetInputElementId("pemilik_kecamatan", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("pemilik_kelurahan"))
		$xt->assign("pemilik_kelurahan_fieldblock",true);
	else
		$xt->assign("pemilik_kelurahan_tabfieldblock",true);
	$xt->assign("pemilik_kelurahan_label",true);
	if(isEnableSection508())
		$xt->assign_section("pemilik_kelurahan_label","<label for=\"".GetInputElementId("pemilik_kelurahan", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("pemilik_kota"))
		$xt->assign("pemilik_kota_fieldblock",true);
	else
		$xt->assign("pemilik_kota_tabfieldblock",true);
	$xt->assign("pemilik_kota_label",true);
	if(isEnableSection508())
		$xt->assign_section("pemilik_kota_label","<label for=\"".GetInputElementId("pemilik_kota", $id, PAGE_EDIT)."\">","</label>");
...
//	pemilik_nama - 
	$value = $pageObject->showDBValue("pemilik_nama", $data, $keylink);
	$showValues["pemilik_nama"] = $value;
	$showFields[] = "pemilik_nama";
		$showRawValues["pemilik_nama"] = substr($data["pemilik_nama"],0,100);

//	pemilik_alamat - 
	$value = $pageObject->showDBValue("pemilik_alamat", $data, $keylink);
	$showValues["pemilik_alamat"] = $value;
	$showFields[] = "pemilik_alamat";
		$showRawValues["pemilik_alamat"] = substr($data["pemilik_alamat"],0,100);

//	pemilik_kecamatan - 
	$value = $pageObject->showDBValue("pemilik_kecamatan", $data, $keylink);
	$showValues["pemilik_kecamatan"] = $value;
	$showFields[] = "pemilik_kecamatan";
		$showRawValues["pemilik_kecamatan"] = substr($data["pemilik_kecamatan"],0,100);

//	pemilik_kelurahan - 
	$value = $pageObject->showDBValue("pemilik_kelurahan", $data, $keylink);
	$showValues["pemilik_kelurahan"] = $value;
	$showFields[] = "pemilik_kelurahan";
		$showRawValues["pemilik_kelurahan"] = substr($data["pemilik_kelurahan"],0,100);

//	pemilik_kota - 
	$value = $pageObject->showDBValue("pemilik_kota", $data, $keylink);
	$showValues["pemilik_kota"] = $value;
	$showFields[] = "pemilik_kota";
		$showRawValues["pemilik_kota"] = substr($data["pemilik_kota"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_reklame";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_reklame_view.php?".$viewlink."'\"");
