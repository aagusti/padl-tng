...
$layout->blocks["top"][] = "details";$page_layouts["app_app_status_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "app_app_status_inline_add.htm";
else
	$templatefile = "app_app_status_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: app_app_status_add.php');
...
//	processing app_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_app_id = $pageObject->getControl("app_id", $id);
		$control_app_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing step - start
	...
	if($inlineAddOption)
	{
		...
		$control_step->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing step - end
//	processing operator - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_operator = $pageObject->getControl("operator", $id);
		$control_operator->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing operator - end
//	processing review - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_review = $pageObject->getControl("review", $id);
		$control_review->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing review - end
//	processing manager - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_manager = $pageObject->getControl("manager", $id);
		$control_manager->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'app_app_status_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'app_app_status_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: app_app_status_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["app_id"]=@$avalues["app_id"];
	$defvalues["step"]=@$avalues["step"];
	$defvalues["operator"]=@$avalues["operator"];
	$defvalues["review"]=@$avalues["review"];
	$defvalues["manager"]=@$avalues["manager"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("app_id_label","<label for=\"".GetInputElementId("app_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("step_label","<label for=\"".GetInputElementId("step", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("operator_label","<label for=\"".GetInputElementId("operator", $id, PAGE_ADD)."\">","</label>");
