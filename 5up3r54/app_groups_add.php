...
$layout->blocks["top"][] = "details";$page_layouts["app_groups_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "app_groups_inline_add.htm";
else
	$templatefile = "app_groups_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: app_groups_add.php');
...
//	processing nama - end
//	processing locked - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_locked = $pageObject->getControl("locked", $id);
		$control_locked->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'app_groups_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'app_groups_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: app_groups_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["locked"]=@$avalues["locked"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("locked_label","<label for=\"".GetInputElementId("locked", $id, PAGE_ADD)."\">","</label>");
