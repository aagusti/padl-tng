...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_jenis_pajak_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_jenis_pajak_inline_add.htm";
else
	$templatefile = "pad_pad_jenis_pajak_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_jenis_pajak_add.php');
...
//	processing rekening_kd_sub - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_rekening_kd_sub = $pageObject->getControl("rekening_kd_sub", $id);
		$control_rekening_kd_sub->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing rekening_kd_sub - end
//	processing rekdenda_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_rekdenda_id = $pageObject->getControl("rekdenda_id", $id);
		$control_rekdenda_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing rekdenda_id - end
//	processing masapajak - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_masapajak = $pageObject->getControl("masapajak", $id);
		$control_masapajak->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing masapajak - end
//	processing jatuhtempo - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jatuhtempo = $pageObject->getControl("jatuhtempo", $id);
		$control_jatuhtempo->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing jalan_kelas_id - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jalan_kelas_id = $pageObject->getControl("jalan_kelas_id", $id);
		$control_jalan_kelas_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_jenis_pajak_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_jenis_pajak_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_jenis_pajak_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["rekening_kd_sub"]=@$avalues["rekening_kd_sub"];
	$defvalues["rekdenda_id"]=@$avalues["rekdenda_id"];
	$defvalues["masapajak"]=@$avalues["masapajak"];
	$defvalues["jatuhtempo"]=@$avalues["jatuhtempo"];
	$defvalues["multiple"]=@$avalues["multiple"];
	$defvalues["jalan_kelas_id"]=@$avalues["jalan_kelas_id"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("rekening_kd_sub_label","<label for=\"".GetInputElementId("rekening_kd_sub", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("rekdenda_id_label","<label for=\"".GetInputElementId("rekdenda_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("masapajak_label","<label for=\"".GetInputElementId("masapajak", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jatuhtempo_label","<label for=\"".GetInputElementId("jatuhtempo", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jalan_kelas_id_label","<label for=\"".GetInputElementId("jalan_kelas_id", $id, PAGE_ADD)."\">","</label>");
