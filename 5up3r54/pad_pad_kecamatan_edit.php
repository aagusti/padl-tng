...
include("include/pad_pad_kecamatan_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_kecamatan_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_kecamatan_inline_edit.htm" : "pad_pad_kecamatan_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_kecamatan_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_kecamatan_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	detail tables
	...
	$showDetailKeys["pad.pad_customer"]["masterkey1"] = $data["id"];		
	$showDetailKeys["pad.pad_daftar"]["masterkey1"] = $data["id"];		
	$showDetailKeys["pad.pad_daftar"]["masterkey2"] = $data["id"];		
	$showDetailKeys["pad.pad_kelurahan"]["masterkey1"] = $data["id"];		
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_kecamatan";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_kecamatan_view.php?".$viewlink."'\"");
