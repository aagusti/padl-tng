...
$layout->blocks["top"][] = "export";$page_layouts["pad_pad_reklame_jalan_export"] = $layout;
...
$xt->display("pad_pad_reklame_jalan_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_jalan.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_jalan.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_jalan.xml");
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=pad_pad_reklame_jalan.csv");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["jalan_kelas_id"]);
				else
					echo $values["jalan_kelas_id"];//echo htmlspecialchars($values["jalan_kelas_id"]); commented for bug #6823
