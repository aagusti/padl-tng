...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_tanah_hit_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_air_tanah_hit_inline_add.htm";
else
	$templatefile = "pad_pad_air_tanah_hit_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_air_tanah_hit_add.php');
...
//	processing vol - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_vol = $pageObject->getControl("vol", $id);
		$control_vol->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing vol - end
...
//	processing vol - start
			...
			if($inlineAddOption)
			{
				$control_vol->afterSuccessfulSave();
			}
//	processing vol - end
//	processing hda - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_air_tanah_hit_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_air_tanah_hit_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_air_tanah_hit_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("vol"))
		$xt->assign("vol_fieldblock",true);
	else
		$xt->assign("vol_tabfieldblock",true);
	$xt->assign("vol_label",true);
	if(isEnableSection508())
		$xt->assign_section("vol_label","<label for=\"".GetInputElementId("vol", $id, PAGE_ADD)."\">","</label>");
...
//	vol
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("vol", $data, $keylink);
		$showValues["vol"] = $value;
		$showFields[] = "vol";
	}	
//	hda
	$display = false;
...
//	update_uid
	...
	{	
		...
		$showRawValues["vol"] = substr($data["vol"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_air_tanah_hit";
		$options['firstTime'] = 1;
