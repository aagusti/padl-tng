...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_air_tanah_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_air_tanah_inline_add.htm";
else
	$templatefile = "pad_pad_air_tanah_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_air_tanah_add.php');
...
//	processing sumur_ke - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sumur_ke = $pageObject->getControl("sumur_ke", $id);
		$control_sumur_ke->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sumur_ke - end
//	processing sipa_no - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_sipa_no = $pageObject->getControl("sipa_no", $id);
		$control_sipa_no->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing sipa_no - end
//	processing awal - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_awal = $pageObject->getControl("awal", $id);
		$control_awal->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing awal - end
//	processing akhir - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_akhir = $pageObject->getControl("akhir", $id);
		$control_akhir->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing akhir - end
...
//	processing awal - start
			...
			if($inlineAddOption)
			{
				$control_awal->afterSuccessfulSave();
			}
//	processing awal - end
//	processing akhir - start
			$inlineAddOption = true;
			if($inlineAddOption)
			{
				$control_akhir->afterSuccessfulSave();
			}
//	processing akhir - end
//	processing volume - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_air_tanah_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_air_tanah_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_air_tanah_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["sumur_ke"]=@$avalues["sumur_ke"];
	$defvalues["sipa_no"]=@$avalues["sipa_no"];
	$defvalues["awal"]=@$avalues["awal"];
	$defvalues["akhir"]=@$avalues["akhir"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("sumur_ke_label","<label for=\"".GetInputElementId("sumur_ke", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("sipa_no_label","<label for=\"".GetInputElementId("sipa_no", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("awal"))
		$xt->assign("awal_fieldblock",true);
	else
		$xt->assign("awal_tabfieldblock",true);
	$xt->assign("awal_label",true);
	if(isEnableSection508())
		$xt->assign_section("awal_label","<label for=\"".GetInputElementId("awal", $id, PAGE_ADD)."\">","</label>");
	
	if(!$pageObject->isAppearOnTabs("akhir"))
		$xt->assign("akhir_fieldblock",true);
	else
		$xt->assign("akhir_tabfieldblock",true);
	$xt->assign("akhir_label",true);
	if(isEnableSection508())
		$xt->assign_section("akhir_label","<label for=\"".GetInputElementId("akhir", $id, PAGE_ADD)."\">","</label>");
...
//	awal
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("awal", $data, $keylink);
		$showValues["awal"] = $value;
		$showFields[] = "awal";
	}	
//	akhir
	$display = false;
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("akhir", $data, $keylink);
		$showValues["akhir"] = $value;
		$showFields[] = "akhir";
	}	
//	volume
	$display = false;
...
//	created
	...
	{	
		...
		$showRawValues["awal"] = substr($data["awal"],0,100);
		$showRawValues["akhir"] = substr($data["akhir"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_air_tanah";
		$options['firstTime'] = 1;
