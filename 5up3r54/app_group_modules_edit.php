...
$layout->blocks["top"][] = "details";$page_layouts["app_group_modules_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "app_group_modules_inline_edit.htm" : "app_group_modules_edit.htm";
...
//	processing group_id - begin
	$condition = 1;

	if($condition)
	{
		$control_group_id = $pageObject->getControl("group_id", $id);
		$control_group_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing group_id - end
//	processing module_id - begin
	$condition = 1;

	if($condition)
	{
		$control_module_id = $pageObject->getControl("module_id", $id);
		$control_module_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing module_id - end
//	processing reads - begin
	$condition = 1;

	if($condition)
	{
		$control_reads = $pageObject->getControl("reads", $id);
		$control_reads->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing reads - end
//	processing writes - begin
	$condition = 1;

	if($condition)
	{
		$control_writes = $pageObject->getControl("writes", $id);
		$control_writes->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing writes - end
//	processing deletes - begin
	$condition = 1;

	if($condition)
	{
		$control_deletes = $pageObject->getControl("deletes", $id);
		$control_deletes->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing deletes - end
//	processing inserts - begin
	$condition = 1;

	if($condition)
	{
		$control_inserts = $pageObject->getControl("inserts", $id);
		$control_inserts->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
	...
	//	do event
		...
		{		
			...
			//	processing group_id - begin
				...
				if($condition)
				{
					$control_group_id->afterSuccessfulSave();
				}
	//	processing group_id - end
			//	processing module_id - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_module_id->afterSuccessfulSave();
				}
	//	processing module_id - end
			//	processing reads - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_reads->afterSuccessfulSave();
				}
	//	processing reads - end
			//	processing writes - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_writes->afterSuccessfulSave();
				}
	//	processing writes - end
			//	processing deletes - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_deletes->afterSuccessfulSave();
				}
	//	processing deletes - end
			//	processing inserts - begin
							$condition = 1;
				...
				if($condition)
				{
					$control_inserts->afterSuccessfulSave();
				}
	//	processing inserts - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: app_group_modules_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: app_group_modules_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["group_id"] = $evalues["group_id"];
	$data["module_id"] = $evalues["module_id"];
	$data["reads"] = $evalues["reads"];
	$data["writes"] = $evalues["writes"];
	$data["deletes"] = $evalues["deletes"];
	$data["inserts"] = $evalues["inserts"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("group_id"))
		$xt->assign("group_id_fieldblock",true);
	else
		$xt->assign("group_id_tabfieldblock",true);
	$xt->assign("group_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("group_id_label","<label for=\"".GetInputElementId("group_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("module_id"))
		$xt->assign("module_id_fieldblock",true);
	else
		$xt->assign("module_id_tabfieldblock",true);
	$xt->assign("module_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("module_id_label","<label for=\"".GetInputElementId("module_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("reads"))
		$xt->assign("reads_fieldblock",true);
	else
		$xt->assign("reads_tabfieldblock",true);
	$xt->assign("reads_label",true);
	if(isEnableSection508())
		$xt->assign_section("reads_label","<label for=\"".GetInputElementId("reads", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("writes"))
		$xt->assign("writes_fieldblock",true);
	else
		$xt->assign("writes_tabfieldblock",true);
	$xt->assign("writes_label",true);
	if(isEnableSection508())
		$xt->assign_section("writes_label","<label for=\"".GetInputElementId("writes", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("deletes"))
		$xt->assign("deletes_fieldblock",true);
	else
		$xt->assign("deletes_tabfieldblock",true);
	$xt->assign("deletes_label",true);
	if(isEnableSection508())
		$xt->assign_section("deletes_label","<label for=\"".GetInputElementId("deletes", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("inserts"))
		$xt->assign("inserts_fieldblock",true);
	else
		$xt->assign("inserts_tabfieldblock",true);
	$xt->assign("inserts_label",true);
	if(isEnableSection508())
		$xt->assign_section("inserts_label","<label for=\"".GetInputElementId("inserts", $id, PAGE_EDIT)."\">","</label>");
...
//	group_id - 
	$value = $pageObject->showDBValue("group_id", $data, $keylink);
	$showValues["group_id"] = $value;
	$showFields[] = "group_id";
		$showRawValues["group_id"] = substr($data["group_id"],0,100);
...
//	module_id - 
	$value = $pageObject->showDBValue("module_id", $data, $keylink);
	$showValues["module_id"] = $value;
	$showFields[] = "module_id";
		$showRawValues["module_id"] = substr($data["module_id"],0,100);

//	reads - 
	$value = $pageObject->showDBValue("reads", $data, $keylink);
	$showValues["reads"] = $value;
	$showFields[] = "reads";
		$showRawValues["reads"] = substr($data["reads"],0,100);

//	writes - 
	$value = $pageObject->showDBValue("writes", $data, $keylink);
	$showValues["writes"] = $value;
	$showFields[] = "writes";
		$showRawValues["writes"] = substr($data["writes"],0,100);

//	deletes - 
	$value = $pageObject->showDBValue("deletes", $data, $keylink);
	$showValues["deletes"] = $value;
	$showFields[] = "deletes";
		$showRawValues["deletes"] = substr($data["deletes"],0,100);

//	inserts - 
	$value = $pageObject->showDBValue("inserts", $data, $keylink);
	$showValues["inserts"] = $value;
	$showFields[] = "inserts";
		$showRawValues["inserts"] = substr($data["inserts"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "app.group_modules";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='app_group_modules_view.php?".$viewlink."'\"");
