...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_terima_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "pad_pad_terima_inline_add.htm";
else
	$templatefile = "pad_pad_terima_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: pad_pad_terima_add.php');
...
//	processing terimano - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_terimano = $pageObject->getControl("terimano", $id);
		$control_terimano->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing jmlterima - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_jmlterima = $pageObject->getControl("jmlterima", $id);
		$control_jmlterima->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing nobukti - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_nobukti = $pageObject->getControl("nobukti", $id);
		$control_nobukti->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'pad_pad_terima_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'pad_pad_terima_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: pad_pad_terima_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("terimano_label","<label for=\"".GetInputElementId("terimano", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("jmlterima_label","<label for=\"".GetInputElementId("jmlterima", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("nobukti_label","<label for=\"".GetInputElementId("nobukti", $id, PAGE_ADD)."\">","</label>");
