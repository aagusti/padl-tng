...
$layout->blocks["top"][] = "details";$page_layouts["pad_pad_spt_type_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "pad_pad_spt_type_inline_edit.htm" : "pad_pad_spt_type_edit.htm";
...
//	processing id - end
//	processing typenm - begin
	$condition = 1;

	if($condition)
	{
		$control_typenm = $pageObject->getControl("typenm", $id);
		$control_typenm->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing typenm - end
...
//	processing write_uid - end
	...
	//	processing id - end
			//	processing typenm - begin
				...
				if($condition)
				{
					$control_typenm->afterSuccessfulSave();
				}
	//	processing typenm - end
			//	processing tmt - begin
							$condition = 1;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: pad_pad_spt_type_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: pad_pad_spt_type_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("typenm"))
		$xt->assign("typenm_fieldblock",true);
	else
		$xt->assign("typenm_tabfieldblock",true);
	$xt->assign("typenm_label",true);
	if(isEnableSection508())
		$xt->assign_section("typenm_label","<label for=\"".GetInputElementId("typenm", $id, PAGE_EDIT)."\">","</label>");
...
//	typenm - 
	$value = $pageObject->showDBValue("typenm", $data, $keylink);
	$showValues["typenm"] = $value;
	$showFields[] = "typenm";
		$showRawValues["typenm"] = substr($data["typenm"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "pad.pad_spt_type";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='pad_pad_spt_type_view.php?".$viewlink."'\"");
