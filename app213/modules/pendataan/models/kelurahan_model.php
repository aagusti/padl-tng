...
class kelurahan_model extends CI_Model {
	...
	function get_all()
	{
		$this->db_pad->order_by('kelurahankd'); 
	...
	{
		$this->db_pad->select('id, kelurahannm');
		$this->db_pad->where(array('enabled'=>'1', 'kecamatan_id'=>$kec_id)); 
		$this->db_pad->order_by('kelurahannm'); 
		$query = $this->db_pad->get($this->tbl);
		if($query->num_rows()!==0)
		{
			return $query->result();
