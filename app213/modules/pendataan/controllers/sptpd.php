...
class sptpd extends CI_Controller
{
    ...
    function grid()
    {
		...
		if (!wp_login()) {
			if ($this->controller == 'sptpd') {
				$this->datatables->select("s.id, get_bayarno(s.id,'pad_spt') as sptno, get_sptno(s.id) as no_sptpd, s.terimatgl, st.typenm,
			...
			if ($this->controller == 'sptpd') {
                ...
                $this->datatables->select("s.id, get_bayarno(s.id,'pad_spt') as bayarno, get_sptno(s.id) as no_sptpd, s.terimatgl,
    ...
    function grid_dokumen() // grid kartu data hotel
    {
        $type_id = $this->uri->segment(4);
        $usaha_id = $this->uri->segment(5);
        ...
        $this->datatables->select("s.id, get_sptno(s.id) as no_sptpd, s.terimatgl, s.type_id, 
                    get_nopd(cu.id, true) as npwpd, cu.opnm, 
            ...
            $this->datatables->rupiah_column('8,9');
            $this->datatables->date_column('2,7');
    ...
    public function edit()
    {
        ...
        */
        ...
        if ($p_id && $get = $this->sptpd_model->get($p_id)) {
            ...
            $tmp_masadari = date('Y-m-d', strtotime($get->masadari));
            $this->session->set_userdata('masadari', $tmp_masadari);
                //set temporary masa dari untuk cek duplikasi masa pajak nantinya
    ...
    public function update()
    {
        ...
        $this->session->set_userdata('masadari','') ;
    }
