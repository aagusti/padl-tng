...
$(document).ready(function() {	
	oTable = $('#table1').dataTable({
		...
		"aoColumnDefs": [			
			...
			{ "aTargets": [2], "bSearchable": true,  "bVisible": true,  "sWidth": "70%", "sClass": "" },
		],
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?=active_module_url();?>reklame_mall/grid"
	}); 
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>reklame_mall/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>reklame_mall/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>reklame_mall/delete/'+mID;
			};
		}else{
