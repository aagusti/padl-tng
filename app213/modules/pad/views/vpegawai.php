...
$(document).ready(function() {	
	oTable = $('#table1').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?=active_module_url();?>pegawai/grid"
	}); 
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>pegawai/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>pegawai/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>pegawai/delete/'+mID;
			};
		}else{
