...
$(document).ready(function() {	
	oTable = $('#table1').dataTable({
		...
		"aoColumnDefs": [			
			...
			{ "aTargets": [6], "bSearchable": true,  "bVisible": true,  "sWidth": "100px", "sClass": "right" },
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?=active_module_url();?>reklame_nsr/grid"
	}); 
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>reklame_nsr/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>reklame_nsr/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>reklame_nsr/delete/'+mID;
			};
		}else{
