...
function hitung_volume() {
	if ($('#meteran_akhir').val() == '') 
		$('#meteran_akhir').autoNumeric('set', 0);
	
	var mAwal = parseInt($('#meteran_awal').autoNumeric('get'));
	mAwal = isNaN(mAwal) ? 0 : mAwal;
	var mAkhir = parseInt($('#meteran_akhir').autoNumeric('get'));
	mAkhir = isNaN(mAkhir) ? 0 : mAkhir;
	...
	var volume = mAkhir - mAwal;
	$('#volume').autoNumeric('set', volume);
}

function get_hda() {
	var zona = $('#air_zona_id').val();
	var manfaat = $('#air_manfaat_id').val();
	
	var volume = $('#volume').autoNumeric('get');
	
	if (volume <= 0) {
		$('#dasar').autoNumeric('set', 0);
		return;
	}
	
	$.ajax({
		url: "<?php echo active_module_url()?>sptpd/get_hda/"+zona+"/"+manfaat+"/"+volume,
		async: false,
		success: function (j) {
			var data = parseFloat(j);
			$('#dasar').autoNumeric('set', data);
			hitung_pajak();
		},
		error: function (xhr, desc, er) {
			alert(er);
...
$(document).ready(function() {
	...
	$('#meteran_awal, #meteran_akhir').keyup(function(event) {
        ...
        if (event.keyCode == 27 || event.keyCode == 13 || (event.keyCode == 65 && event.ctrlKey === true) || 
...
/* 	$('#meteran_awal, #meteran_akhir').keypress(function(event) {
		hitung_volume();
	...
	$('#air_zona_id, #air_manfaat_id').change(function() {
		get_hda();
	});
		...
		<?php echo form_open($faction, array('id'=>'myform','class'=>'form-horizontal'));?><br>
			...
			<div class="row">
				<div class="span4" style="width:300px;">
					<div class="form-group">
						<label class="control-label col-sm-1" for="air_zona_id">Zona</label>
						<div class="col-sm-4">
							<?=$select_zona;?>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-1" for="air_manfaat_id">Klasifikasi</label>
						<div class="col-sm-4">
							<?=$select_manfaat;?>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-1" for="meteran_awal">Meteran Awal/Akhir</label>
						<div class="col-sm-4">
							<input class="form-control" type="text" style="text-align:right;" name="meteran_awal" id="meteran_awal" value="<?=$dt['meteran_awal']?>" required /> / 
							<input class="form-control" type="text" style="text-align:right;" name="meteran_akhir" id="meteran_akhir" value="<?=$dt['meteran_akhir']?>" required />
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-1" for="volume">Volume (M<sup>3</sup>)</label>
						<div class="col-sm-4">
