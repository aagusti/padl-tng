...
$(document).ready(function() {	
	oTable = $('#table1').dataTable({
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "<?=active_module_url();?>rek_nsr/grid"
	}); 
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>rek_nsr/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>rek_nsr/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>rek_nsr/delete/'+mID;
			};
		}else{
