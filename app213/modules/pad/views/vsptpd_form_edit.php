...
function get_pajak_detail() {
	...
	document.getElementById('masapajak_bulan').click();
}
...
$(document).ready(function() {
	function get_cu_edit(customer_id, cu_id, def_pajak_id) {
		$.ajax({
			url: "<?php echo active_module_url()?>sptpd/get_cu_edit/"+customer_id+"/"+cu_id+"/"+def_pajak_id,
			async: false,
			success: function (j) {
				var data = $.parseJSON(j);
...
$('#nopd').typeahead({
	...
	updater: function(item) {
		...
		get_cu_edit(parts[2], parts[0], parts[6]);		
		$('#customer_usaha_id').trigger('change');
		//get_pajak_detail();
						...
						<div class="row col-sm-12">	
							<div class="row col-sm-6">
								<div class="form-group">
									...
									<div class="col-sm-2">
										<button type="button" class="btn btn-success" data-toggle="modal" data-target="#omsetDialog">Input Omset</button>
									</div>
								</div>	
