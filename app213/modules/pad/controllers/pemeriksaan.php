...
class pemeriksaan extends CI_Controller
{
    ...
    private $controller = 'pemeriksaan';
    ...
    public function index()
    {
        ...
        $js = 'id="proses_id" class="form-group" style="width:150px;"';
       ...
       $options = array(
        ...
        $this->load->view(active_module().'/vpemeriksaan', $data);
    }
    ...
    function grid()
    {
        ...
        if ($proses_id == 2) {
            $this->datatables->select("id, tahun, periksa_no, periksa_tgl, lhp_no, keterangan, pokok, denda, bunga, total", false);
            $this->datatables->from('pad_periksa');
            $this->datatables->rupiah_column('6,7,8, 9');
        } else {
            $this->datatables->select("id, tahun, periksa_no, periksa_tgl, lhp_no, keterangan, pokok, denda, bunga, total", false);
            $this->datatables->from('pad_periksa');
        }
        ...
        if ($spt_id && $sptpd = $this->sptpd_model->get($spt_id)) {            
            ...
            if($this->sspd_model->save($post_data)) {
             ...
             if($sisa>0){
                ...
                $created = date('Y-m-d');
                $create_uid = sipkd_user_id();
                $this->db->query("insert into pad_stpd (tahun, stpdno, stpdtgl, invoice_id, jatuhtempotgl, stpdcount, bunga, create_uid, created) 
                                               values ($tahun, $stpdno,'$prosestgl', $inv_id, '$jatuhtempotgl', $stpdcount, $bunga, $create_uid, '$created')");
                //INSERT INVOICE
                $query = $this->db->query("select id from pad_stpd where invoice_id = $inv_id 
                          order by invoice_id desc limit 1 ");
                foreach ($query->result() as $row){
                        $stpd_id = $row->id;
                    }
                ...
                $query = $this->db->query(" select s.id as source_id, 'pad_stpd' as source_nama, inv.tahun, inv.usaha_id, get_invno(inv.source_id) as invoice_no,                                                                                                   
                                            ...
                                            from pad_stpd s 
                                            inner join pad_invoice inv on s.invoice_id = inv.id                                                                                                 
                                            where s.id= $stpd_id");
    ...
    public function batal_sspd()
    {
        ...
        $spt_id   = $this->uri->segment(4);
        if ($spt_id && $sptpd = $this->sptpd_model->get($spt_id)) {
            // jangan dihapus katanya
            // $this->sspd_model->delete_by_spt($spt_id);       
            $this->sspd_model->cancel_by_spt($spt_id);      
